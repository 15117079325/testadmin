...
class ServerAPI{
    ...
    public function __construct(){
        //product
          $this->appKey = 'pgyu6atqp5z8u';
          $this->appSecret = 'CuPW8Ngmzk';
        //test
//       $this->appKey = 'cpj2xarlctv6n';
//       $this->appSecret = '6C9b9kdCLaeZE';
...
//                throw new Exception('禁言时长 不能为空');
    ...
    public function groupRefresh($groupId, $groupName) {
        try{
            ...
            $ret = $this->curl('/group/refresh',$params,'urlencoded','im','POST');
            if(empty($ret))
 ...
 */
    public function groupBanAdd($groupId) {
        try{
            if(empty($groupId))
                throw new Exception('群组 Id 不能为空');
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/ban/add',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
    ...
    public function groupBanRollback($groupId) {
        try{
            if(empty($groupId))
                throw new Exception('群组 Id 不能为空');
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/ban/rollback',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
    ...
    public function groupBanQuery($groupId) {
        try{
            if(empty($groupId))
                throw new Exception('群组 Id 不能为空');
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/ban/query',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    public function groupUserBanWhitelistAdd($userId,$groupId) {
        try{
            if(empty($userId))
                throw new Exception('用户 Id 不能为空');
            if(empty($groupId))
            ...
            $params['userId'] = $userId;
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/user/ban/whitelist/add',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    public function groupUserBanWhitelistRollback($userId,$groupId) {
        try{
            if(empty($userId))
                throw new Exception('用户 Id 不能为空');
            if(empty($groupId))
            ...
            $params['userId'] = $userId;
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/user/ban/whitelist/rollback',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    public function groupUserBanWhitelistQuery($groupId) {
        try{
            if(empty($groupId))
                throw new Exception('群组 Id 不能为空');
            $params['groupId'] = $groupId;
            $ret = $this->curl('/group/user/ban/whitelist/query',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
