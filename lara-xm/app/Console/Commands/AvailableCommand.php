...
class AvailableCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'huodan:available';
     ...
     * @var string
     */
    protected $description = '每日报单消耗可用券';
    ...
    {
        //
        $code = config('adminmaster.statis.available_statis_count');
        ...
        $giftSql = "SELECT SUM(balance_money) AS num,FROM_UNIXTIME(create_at,'%Y-%m-%d') AS cus_time FROM xm_customs_order WHERE FROM_UNIXTIME(create_at,'%Y-%m-%d')>=? AND FROM_UNIXTIME(create_at,'%Y-%m-%d')<? GROUP BY cus_time";
        ...
        if ($result) {
            exit('脚本执行完毕，用户每日报单消耗可用券统计完毕' . PHP_EOL);
        } else {
            exit('脚本执行完毕，用户每日报单消耗可用券统计失败' . PHP_EOL);
        }
    }
}
