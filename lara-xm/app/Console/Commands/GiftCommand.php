...
class GiftCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'huodan:gift';
     ...
     * @var string
     */
    protected $description = '每日礼包专区报单金额（现金部分）';
    ...
    {
        //
        $code = config('adminmaster.statis.gift_statis_count');
        ...
        $giftSql = "SELECT SUM(customs_money) AS num,FROM_UNIXTIME(create_at,'%Y-%m-%d') AS cus_time FROM xm_customs_order WHERE balance_money!=0 AND customs_money!=0 AND FROM_UNIXTIME(create_at,'%Y-%m-%d')>=? AND FROM_UNIXTIME(create_at,'%Y-%m-%d')<? GROUP BY cus_time";
