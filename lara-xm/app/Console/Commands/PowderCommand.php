...
class PowderCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'huodan:powder';
     ...
     * @var string
     */
    protected $description = '火粉社区每日交易总额';
    ...
    public function handle()
    {
        //
        //每日新增用户
        $code = config('adminmaster.statis.trade_statis_community');
...
//        SELECT
//	SUM( origin_trade_num ) AS num,
//	FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) AS cus_time
//FROM
//	xm_trade
//WHERE
//	trade_status = 2
//    AND FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) >= '2019-09-22'
//    AND FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) < '2019-09-29'
//GROUP BY
//	cus_time


        $registerSql = "SELECT SUM( origin_trade_num ) AS num,FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) AS register_time FROM xm_trade WHERE trade_status = 2 AND FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) >=? AND FROM_UNIXTIME( trade_gmt_create, '%Y-%m-%d' ) <? GROUP BY register_time";
        ...
        if ($result) {
            exit('脚本执行完毕，火粉社区每日交易总额统计完毕' . PHP_EOL);
        } else {
            exit('脚本执行完毕，火粉社区每日交易总额统计失败' . PHP_EOL);
        }
    }
}
