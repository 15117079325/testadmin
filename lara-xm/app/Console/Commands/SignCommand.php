...
class SignCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'huodan:sign';
    ...
    {
        //
        $code = config('adminmaster.statis.sign_statis_count');
        ...
        $giftSql = "SELECT SUM(amount) AS num,FROM_UNIXTIME(create_at,'%Y-%m-%d') AS cus_time FROM xm_flow_log WHERE type=2 AND status=1 AND target_type=4 AND FROM_UNIXTIME(create_at,'%Y-%m-%d')>=? AND FROM_UNIXTIME(create_at,'%Y-%m-%d')<? GROUP BY cus_time";
        ...
        if ($result) {
            exit('脚本执行完毕，用户每日签到产生券统计完毕' . PHP_EOL);
        } else {
            exit('脚本执行完毕，用户每日签到产生券统计失败' . PHP_EOL);
        }
    }
}
