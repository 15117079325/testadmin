...
//		$result_ = strtolower($String);
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_CAINFO, WxPayConf_pub::CAINFO_PATH);
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null) 
			{
				...
				// 统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！"."<br>
			}
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：生成可以获得code的url
	 */
	function createOauthUrlForCode($redirectUrl,$userfromid)
	{
		$urlObj["appid"] = WxPayConf_pub::APPID;
		$urlObj["redirect_uri"] = "$redirectUrl";
		//$urlObj["userfromid"] = "$userfromid";
	 ...
	 */
	function createOauthUrlForCodeByUserinfo($redirectUrl,$userfromid)
	{
		$urlObj["appid"] = WxPayConf_pub::APPID;
		$urlObj["redirect_uri"] = "$redirectUrl";
		//$urlObj["userfromid"] = "$userfromid";
	 ...
	 */
	function getAccessTokenAndOpenid()
	{
		$url = $this->createOauthUrlForOpenid();
        //初始化curl
       	$ch = curl_init();
	...
	/**
	 * 	作用：请求获取accesstoken和 openid  直接数组的形式返回
	 */
	function getAccessTokenAndOpenidArray()
	{
		$url = $this->createOauthUrlForOpenid();
        //初始化curl
       	$ch = curl_init();
	...
	/**
	 * 	作用：通过access_token 和  openId获取用户的信息 会弹出确认登陆的方式
	 */
	function getUserInfo()
	{
		$params = $this->getAccessTokenAndOpenid();
	...
	/**
	 * 	作用：通过access_token 和  openId获取用户的信息  不弹出确认登陆的方式
	 */
	function getUserInfoUnVerify($openid="",$access_token="")
	{
