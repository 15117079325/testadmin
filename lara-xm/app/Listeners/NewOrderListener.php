...
//            $this->bonusWeishang($user_extra, $customs_order->customs_money, $team_config, $user_name, $customs_order->order_id, 1);
        ...
        while (true) {
                ...
                while (true) {
                    ...
                    if ($user_extra2->user_cx_rank == $last_rank) {
                        ...
                        $insert_data = [
                            ...
                            'type' => 3,
                            'status' => 2,
                            'notes' => $user_name . '新人专区购买释放团队优惠券' . $amount1,
                            'create_at' => $now,
                            'target_type' => $target_type,
                            'target_id' => $target_id
                        ];
            ...
            if (in_array($user_extra2->user_cx_rank, [2, 3, 4])) {
                if ($user_extra2->user_cx_rank > $last_rank) {
                    ...
                    $gold_pool = DB::table('user_account')->where('user_id', $user_extra2->user_id)->pluck('balance')->first();
                    $notes = $user_name . '新人专区购买获得团队奖释放' . $reward_money;
                    ...
                    $insert_data = [
                        ...
                        'type' => 3,
                        'status' => 2,
                        'notes' => $user_name . '新人专区购买释放团队优惠券' . $reward_money,
                        'create_at' => $now,
                        'target_type' => $target_type,
                        'target_id' => $target_id
                    ];
    ...
    public function countNum($invie_user_id, $direct_assess_num)
    {
        // //天数
        // $day = 30;
        // //算出距离的时间
    ...
    public function Upacc($money = 0, $user_id = 0, $user_name, $customs_order, $customs_config)
    {
        ...
        $num = isset($acc->release_balance) ? $money - $acc->release_balance : 0;
        //没有抵消完继续进行
        if ($num > 0) {
            //得到要释放的优惠券金额
...
//                'balance' => $acc->balance + $money1,
                'temporary_balance' => $acc->temporary_balance + $money1,
            ];
            ...
            //插入日志
            $notes = $user_name . '新人专区购买优惠券释放' . $money1;
            $insert_data = [
                'user_id' => $user_id,
                'amount' => $money1,
            ...
            $insert_data = [
                ...
                'type' => 3,
                'status' => 2,
                'notes' => $user_name . '新人专区购买释放优惠券' . $money1,
                'create_at' => $now,
                'target_type' => 1,
                'target_id' => $customs_order->order_id
            ];
            ...
            $date = [
                'release_balance' => isset($acc->release_balance) ? $acc->release_balance - $money : 0,
//                'balance' => $acc->balance + $money,
                'temporary_balance' => isset($acc->temporary_balance) ? $acc->temporary_balance + $money : $money,
            ];
            ...
            //插入日志
            $notes = $user_name . '新人专区购买优惠券释放' . $money;
            $insert_data = [
                'user_id' => $user_id,
                'amount' => $money,
                'surplus' => isset($acc2->release_balance) ? $acc2->release_balance : 0,
                'type' => 2,
                'status' => 1,
                'notes' => $notes,
                'create_at' => $now,
            ...
            $insert_data = [
                ...
                'type' => 3,
                'status' => 2,
                'notes' => $user_name . '新人专区购买释放优惠券' . $money,
                'create_at' => $now,
                'target_type' => 1,
                'target_id' => $customs_order->order_id
            ];
    ...
    public function manage_acc($money = 0, $user_id = 0, $user_name, $customs_order, $customs_config)
    {
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            if ($money / 2 >= 1) {
                ...
                //重新给变量新的用户ID
                $user_id = isset($user->invite_user_id) ? $user->invite_user_id : 0;
                ...
                // }
                //算出待释放金额是否可以完全释放
                $num1 = isset($acc1->release_balance) ? $acc1->release_balance - $superior : 0;
...
//                DB::update('UPDATE xm_user_account SET balance = balance + ?,release_balance = release_balance - ?,update_at = ? WHERE user_id = ?', [$superior, $superior, $now, $user_id]);
                DB::update('UPDATE xm_user_account SET temporary_balance = temporary_balance + ?,release_balance = release_balance - ?,update_at = ? WHERE user_id = ?', [$superior, $superior, $now, $user_id]);
