...
{
	echo '<center><br>请在提交商品后编辑模式下添加商品相册！</center>';
	exit;
}
//echo $goods_attr_id;
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    {
        //@unlink('../' . $row['img_url']);
        \maqu\filesystem\FileSystemManager::getAdapter()->deleteFile($row['img_url']);
    }
    if ($row['thumb_url'] != '' && is_file('../' . $row['thumb_url']))
    {
//        @unlink('../' . $row['thumb_url']);
        \maqu\filesystem\FileSystemManager::getAdapter()->deleteFile($row['thumb_url']);
    }
    if ($row['img_original'] != '' && is_file('../' . $row['img_original']))
    {
//        @unlink('../' . $row['img_original']);
        \maqu\filesystem\FileSystemManager::getAdapter()->deleteFile($row['img_original']);
    }
...
<script type="text/javascript" src="./js/diyUpload/js/webuploader.html5only.js"></script>
...
if ($img_list)
{
	...
	{
        $img['thumb_url'] = \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($img['thumb_url']);
...
function handle_gallery_image_attr($goods_id, $goods_attr_id, $image_files)
{
    ...
    $proc_thumb = (isset($GLOBALS['shop_id']) && $GLOBALS['shop_id'] > 0)? false : true;
        ...
        if ($flag)
        {
            ...
            $img_url = \maqu\filesystem\FileSystemManager::getAdapter()->uploadFile(ROOT_PATH . $img_url,$thumb_url);
...
//                @unlink('../' . $img_original);
                \maqu\filesystem\FileSystemManager::getAdapter()->deleteFile($img_original);
            }
        }
