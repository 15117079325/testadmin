...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    /*add by ymm 2017-2-23*/
    /*--------------根据易物类型统计总和*/
    $sums = '';
    if($goods_list[0]['barter_type'] == 1){
        $sums = $goods_list[0]['shopping_money'];
    }elseif($goods_list[0]['barter_type'] == 2){
        $sums = $goods_list[0]['consume_money']+$goods_list[0]['cash_money'];
    }elseif($goods_list[0]['barter_type'] == 3){
        $sums = null;
    }
    /* 模板赋值 */
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    if (isset($_POST['confirm']))
    {
        $status_back='1';
		  update_back($back_id, $status_back, $status_refund);
         back_action($back_id, $status_back, $order['status_refund'],"确认商品退回");
        //add 收到退货并退款
        $status_refund = '1';
        $order = back_order_info($back_id);
        ...
        $db->query($sql);
        //var_dump($order);die();
        $sql = "update ". $ecs->table('back_order') ." set  status_refund='$status_refund', refund_type = 1  where back_id='$back_id' ";
        $db->query($sql);
        if($order['barter_type == 1']){
            $account_type = array(
                'shopping'=>$order['shopping_money_all']
            );
        }else{
            $account_type = array(
                'cash'=>$order['cash_money_all'],
                'consume'=>$order['consume_money_all']
            );
        }

        $sql="update ". $GLOBALS['ecs']->table('back_goods') ." set status_back='$status_back' where back_type = 1 and back_id='$back_id' ";
        //var_dump($sql);die();
        $GLOBALS['db']->query($sql);
        /*--------更新个人账户表及写入日志*/
        $sql = "SELECT a.account_id FROM " . $ecs->table('mq_users_extra') . " as a WHERE user_id = " . $order['user_id'];
        $account_id = $GLOBALS['db']->getRow($sql);
        foreach ($account_type AS $k=>$v) {
            /*---------更新账户--------*/
            $sqlone = "UPDATE " . $ecs->table('mq_account') . "
            SET money = money + " . $v . "
            WHERE account_type = '" . $k . "' AND account_id = '" . $account_id['account_id'] . "'";
            $db->query($sqlone);
            $user_id = $order['user_id'] ;
            $sql = "SELECT IFNULL(ma1.money,0) as cash_credit, IFNULL(ma2.money,0) as consume_credit, IFNULL(ma3.money,0) as shopping_credit ".
            ...
            $casha =  $GLOBALS['db']->getRow($sql);
            if($k == "shopping"){
                $money = $casha['shopping_credit'];
            }elseif($k == "cash"){
                $money = $casha['cash_credit'];
            }else{
                $money = $casha['consume_credit'];
            }
            /*----------写入日志-------------*/
            $sqltwo = "INSERT INTO " . $ecs->table('mq_account_log') . "
            (account_id,account_type,change_money,income_type,change_time,change_desc,change_type,money)
            VALUES ('" . $account_id['account_id'] . "' , '" . $k . "','" . $v . "','+','" . time() . "','退货退款','11', '".$money."') ";
            $db->query($sqltwo);
        }
        back_action($back_id, $order['status_back'], $status_refund,"确认退款");
        $status_back='3';
        update_back($back_id, $status_back, $status_refund);
        back_action($back_id, $status_back, $status_refund, "确认完成退货");
...
//	$sql = "update ". $ecs->table('back_goods') ." set status_refund='$status_refund'  where back_id='$back_id' and (back_type='0' or back_type='4') ";
//	$db->query($sql);
//    var_dump($_REQUEST['refund_type']);die();
...
//        $GLOBALS['db']->query($sql);
//        /*---订单号*/
//        $sql_order_number = "SELECT o.order_sn ,u.user_name fROM ".$ecs->table('back_order')." AS o
//        LEFT JOIN ".$ecs->table('users')." AS u ON o.user_id = u.user_id
//        WHERE o.order_id = ".$order['order_id'];
//        $order_sn_name = $GLOBALS['db']->getRow($sql_order_number);
//        /*------日志内容--*/
//        $change_desc = $order_sn_name['user_name'].'的一笔退货单,单号是'.$order_sn_name['order_sn'] ;
//        /*--------更新个人账户表及写入日志*/
//        $sql = "SELECT a.account_id FROM " . $ecs->table('mq_users_extra') . " as a WHERE user_id = " . $order['user_id'];
//        $account_id = $GLOBALS['db']->getRow($sql);
//        foreach ($account_type AS $k=>$v) {
...
//            /*----------写入日志-------------*/
//            $sqltwo = "INSERT INTO " .$ecs->table('mq_account_log'). "
//            (account_id,account_type,change_money,income_type,change_time,change_desc)
//            VALUES ('". $account_id['account_id'] ."' , '". $k ."','". $v ."','+','". time() ."','". $change_desc ."') ";
//            $db->query($sqlone);
//            $db->query($sqltwo);
//        }
//		//是否开启优惠券变动给客户发短信-退款
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        if($filter['supp']){
        	$sql = "SELECT b.*,s.supplier_name FROM " . $GLOBALS['ecs']->table("back_order") . "  $where   ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order'].
...
function back_action($back_id, $status_back, $status_refund,  $note = '', $username = null)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('back_action') .
            ...
            'SELECT ' .
                "$back_id, '$username', '$status_back', '$status_refund',  '$note', '" .time() . "' " .
