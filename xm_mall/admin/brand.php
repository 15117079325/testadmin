...
elseif ($_REQUEST['act'] == 'insert')
{
   ...
   $db->query($sql);
   $id = $db->insert_id();
    admin_log($_POST['brand_name'],'add','brand',BRAND_ID.$id);
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    if ($exc->edit($param,  $_POST['id']))
    {
        ...
        admin_log($_POST['brand_name'], 'edit', 'brand',BRAND_ID.$id);
...
elseif ($_REQUEST['act'] == 'edit_brand_name')
{
    ...
    else
    {
        ...
        {
            admin_log($name,'edit','brand',BRAND_ID.$id);
...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    {
        admin_log(addslashes($name),'edit','brand',BRAND_ID.$id);
...
//        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
        \maqu\filesystem\FileSystemManager::getAdapter()->deleteFile(DATA_DIR . '/brandlogo/' .$logo_name);
    }
...
//            '<a href="../' . DATA_DIR . '/brandlogo/'.$rows['brand_logo'].'" target="_brank"><img src="images/picflag.gif" width="16" height="16" border="0" alt='.$GLOBALS['_LANG']['brand_logo'].' /></a>';
        ...
        $fullpath = \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath(DATA_DIR . '/brandlogo/'.$rows['brand_logo']);
        $brand_logo = empty($rows['brand_logo']) ? '' :
            '<a href="' .  $fullpath .'" target="_brank"><img src="images/picflag.gif" width="16" height="16" border="0" alt='.$GLOBALS['_LANG']['brand_logo'].' /></a>';
