...
function get_comment_list()
{
    ...
    $filter['sort_by']      = empty($_REQUEST['sort_by']) ? 'add_time' : htmlspecialchars(trim($_REQUEST['sort_by']));
    $filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'DESC' : htmlspecialchars(trim($_REQUEST['sort_order']));

    if(!empty($filter['keywords'])){
       $where="AND content LIKE '%" . mysql_like_quote($filter['keywords']) . "%' ";
    ...
    if(!empty($_REQUEST['keywords'])){
        ...
        $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 
             ORDER BY $filter[sort_by] $filter[sort_order] ".
            " LIMIT ". $filter['start'] .", $filter[page_size]";
        $res  = $GLOBALS['db']->query($sql);
