...
define('APPKEY' , 'RMIVjtHJv595BWNcvbWsq5');
define('APPID','2n2aPk9Zrk6z8T8UzJJhL8');
define('MASTERSECRET','OUyVwmztR88LlVjFB98dO8');
...
//    $message = new IGtSingleMessage();
    ...
    //单推接口案例
    public function pushMessageToSingle($clientid,$custom_content)
    {
        $igt = new IGeTui(HOST, APPKEY, MASTERSECRET);
...
//        $custom_content_arr = json_decode($custom_content,true);
        ...
        $template = $this->IGtTransmissionTemplateDemo($title,$content,$custom_content);
        ...
        if ($content == '您的账号已被强制退出' || strpos($content, '已被拉黑') !== false) {
            $message->set_offlineExpireTime(1000 * 24 * 3600 * 30);//离线时间
        } else {
...
//	$message->set_PushNetWorkType(0);//设置是否根据WIFI推送消息，1为wifi推送，0为不限制推送
        ...
        // $target->set_clientId('341d3ceaed4aae84e9ea9ed8e5b00a59');

        // echo "11111";
        // $target->set_clientId('ae80e31065ef084ddd0337eda2290965');
...
//    $target1->set_alias(Alias);
    ...
    //群推接口案例
    public function pushMessageToApp($custom_content)
    {
        $custom_content_arr = json_decode($custom_content,true);
        $title = $custom_content_arr['title'];
        $content = $custom_content_arr['content'];
        $igt = new IGeTui(HOST, APPKEY, MASTERSECRET);
        $template = $this->IGtTransmissionTemplateDemo($title,$content,$custom_content);
...
//        $template->set_apnInfo($apn);
    ...
    public function IGtTransmissionTemplateDemo($title,$content,$custom_content)
    {
        $template = new IGtTransmissionTemplate();
        $template->set_appId(APPID);//应用appid
        $template->set_appkey(APPKEY);//应用appkey
        $template->set_transmissionType(2);//透传消息类型
        // $template->set_transmissionContent("DZK");//透传内容
        $custom_content_val = json_encode($custom_content);
        $template->set_transmissionContent($custom_content_val);//透传内容
        //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
        //APN简单推送
//        $template = new IGtAPNTemplate();
//        $apn = new IGtAPNPayload();
...
//        IOS8.2 支持
        $alertmsg->title = $title;
        // $alertmsg->titleLocKey="TitleLocKey";
        $alertmsg->titleLocKey = $content;
        $alertmsg->titleLocArgs = array("TitleLocArg");

        $apn->alertMsg = $alertmsg;
        $apn->badge = "";
        $apn->sound = "";
        $apn->add_customMsg("custom_content", $custom_content);
