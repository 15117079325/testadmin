...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    if(isset($image_descs) && !empty($image_descs) && is_array($image_descs)){
...
//                $thumb_url = is_string($thumb_url) ? $thumb_url : '';
                    $thumb_url = \maqu\filesystem\FileSystemManager::getAdapter()->uploadFile(ROOT_PATH . $thumb_url,$thumb_url);
                }
...
function update_goods($goods_id, $field, $value)
{
    if ($goods_id)
    {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('goods') .
                " SET $field = '$value' , last_update = '". time() ."' " .
 ...
 * @return  string  唯一的货号
 */
function new_generate_goods_sn($goods_id)
{
    $goods_sn = $GLOBALS['_CFG']['sn_prefix'] . str_repeat('0', 6 - strlen($goods_id)) . $goods_id;

    $sql = "SELECT p_sn FROM " . $GLOBALS['ecs']->table('product') .
            " WHERE p_sn LIKE '" . mysql_like_quote($goods_sn) . "%' AND p_id <> '$goods_id' " .
            " ORDER BY LENGTH(p_sn) DESC";
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
     $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type,a.attr_txm, a.attr_values, v.attr_value, v.attr_price,v.attr_shopping_credit,v.attr_cash_credit,v.attr_consume_credit ".
 ...
 * @return  array
 */
function yiwu_goods_list($is_delete, $real_goods=1, $conditions = '')
{
    /* 过滤条件 */
    $param_str = '-' . $is_delete . '-' . $real_goods;
    $result = get_filter($param_str);
...
//        $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
        $today = local_mktime($day['hours'], $day['minutes'], 59, $day['mon'], $day['mday'], $day['year']);
        /* 代码修改 By  www.68ecshop.com 促销商品时间精确到时分 End */
        $filter['allow_bb']           = empty($_REQUEST['allow_bb']) ? 0 : intval($_REQUEST['allow_bb']);
        $filter['cat_id']           = empty($_REQUEST['cat_id']) ? 0 : intval($_REQUEST['cat_id']);
        $filter['intro_type']       = empty($_REQUEST['intro_type']) ? '' : trim($_REQUEST['intro_type']);
        $filter['is_promote']       = empty($_REQUEST['is_promote']) ? 0 : intval($_REQUEST['is_promote']);
        $filter['stock_warning']    = empty($_REQUEST['stock_warning']) ? 0 : intval($_REQUEST['stock_warning']);
...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
        ...
        /* 代码修改 By  www.68ecshop.com End */
        $filter['suppliers_id'] = isset($_REQUEST['suppliers_id']) ? (empty($_REQUEST['suppliers_id']) ? '' : trim($_REQUEST['suppliers_id'])) : '';
        //edit shengl 2017年2月9日17:31:31
        $filter['referee_id'] = isset($_REQUEST['referee_id']) ? (empty($_REQUEST['referee_id']) ? '' : trim($_REQUEST['referee_id'])) : '';
        //end
        $filter['is_on_sale'] = isset($_REQUEST['is_on_sale']) ? ((empty($_REQUEST['is_on_sale']) && $_REQUEST['is_on_sale'] === 0) ? '' : trim($_REQUEST['is_on_sale'])) : '';
        if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
        {
        ...
        $where = $filter['cat_id'] > 0 ? " AND " . get_children($filter['cat_id']) : '';
        /* 是否H单 */
        switch ($filter['allow_bb'])
        {
            case 0:
                break;
            case 1:
                $where .= ' AND mge.allow_bb=0';
                break;
            case 2:
                $where .= ' AND mge.allow_bb=1';
                break;
            default:
                break;
        }
        ...
        if (!empty($filter['referee_id']))
        {
            $where .= " AND (referee_id = '" . $filter['referee_id'] . "')";
        }
        $where .= $conditions;

        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g LEFT JOIN ".$GLOBALS['ecs']->table('mq_goods_extra')." AS mge ON g.goods_id = mge.goods_id WHERE is_delete='$is_delete' $where";
        ...
        $sql = "SELECT g.goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
            " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
            ", g.suppliers_id, s.suppliers_name, mge.belongto, mge.shopping_credit, mge.cash_credit, mge.consume_credit, mge.referee_id, IFNULL(mge.referee_percent,'0') AS referee_percent, u.real_name as referee_name ".
            " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".
            " LEFT JOIN " . $GLOBALS['ecs']->table('suppliers') . " AS s ON g.suppliers_id = s.suppliers_id " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('mq_goods_extra') . " AS mge ON mge.goods_id = g.goods_id " .
            " LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON mge.referee_id = u.user_id " .
            " WHERE is_delete='$is_delete' $where" .
            " ORDER BY $filter[sort_by] $filter[sort_order] ".
            " LIMIT " . $filter['start'] . ",$filter[page_size]";
 ...
 * @return  string
 */
function build_big_yiwu_attr_html($cat_id, $goods_id = 0 , $bar_code = 0)
{
    $attr = get_attr_list($cat_id, $goods_id);
    $html = '<table width="100%" id="attrTable">';
    $spec = 0;
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            '购物券'.' <input type="text" name="attr_shopping_credit_list[]" value="' . $val['attr_shopping_credit'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_shopping_credit_list[]" value="0" />';
 ...
 * @return  string
 */
function build_mid_yiwu_attr_html($cat_id, $goods_id = 0 , $bar_code = 0)
{
    $attr = get_attr_list($cat_id, $goods_id);
    $html = '<table width="100%" id="attrTable">';
    $spec = 0;
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            '该属性 新美积分价格 <input type="text" name="attr_cash_credit_list[]" value="' . $val['attr_cash_credit'] . '" size="5" maxlength="10" />&nbsp;+&nbsp;消费积分价格 <input type="text" name="attr_consume_credit_list[]" value="' . $val['attr_consume_credit'] . '" size="5" maxlength="10" />' :
            ' <input type="hidden" name="attr_cash_credit_list[]" value="0" /><input type="hidden" name="attr_consume_credit_list[]" value="0" />';
