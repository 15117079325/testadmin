 ...
 * @return  void
 */
function admin_log($sn = '', $action, $content,$msg='')
{
    //如果是自己定义的内容
    if($action == 1) {
        $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
            " VALUES ('" . time() . "', $_SESSION[admin_id], '" . $msg . "', '" . real_ip() . "')";
    }else if(!$msg){
        ...
        $log_info = $GLOBALS['_LANG']['log_action'][$action] . $GLOBALS['_LANG']['log_action'][$content] .': '.addslashes($sn).','.$msg;
        $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
            " VALUES ('" . time() . "', $_SESSION[admin_id], '" . stripslashes($log_info) . "', '" . real_ip() . "')";
    }
...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    {
        if($priv_str == 'mq_dash_board') {
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'index.php?act=%27%27');
            die;
        } else {
 ...
 */
function create_html_editor($input_name, $input_value = '', $toolbars = '',$width='',$height="")
{
    global $smarty;
    ...
    toolbars:[
        [
            ...
            \'bold\', \'italic\', \'underline\', \'fontborder\', \'strikethrough\', \'blockquote\', \'pasteplain\', \'|\',
            \'forecolor\', \'backcolor\', \'insertorderedlist\', \'insertunorderedlist\', \'selectall\', \'cleardoc\', \'|\',
            \'link\', \'|\', \'background\', \'|\', \'horizontal\', \'|\',
...
EOF;
    ...
    $imagePath = '';
    if(FILESYSTEM_DRIVER == 'alioss'){
        $res_url = 'https://' . STATIC_RESOURCE_URL . '/';
        $imagePath = <<<EOF
imagePath:'%s',
scrawlPath:'%s',
filePath:'%s',
...
videoPath:'%s'
EOF;
        $imagePath = sprintf($imagePath,$res_url,$res_url,$res_url,$res_url,
            $res_url,$res_url,$res_url,$res_url);
    }
    $html = sprintf($html,$input_name,$input_name,$input_value,$input_name,$width,$height,$imagePath, $toolbars);
 ...
 * @return      array
 */
function referee_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
    ...
    /* 查询 */
    $sql = "SELECT U.user_id, U.real_name
            FROM " . $GLOBALS['ecs']->table("mq_users_extra") . " AS MUE
            LEFT JOIN " . $GLOBALS['ecs']->table("users") . " AS U
            ON MUE.user_id = U.user_id and MUE.is_referee = 1
            $where";
 ...
 * @return  array
 */
function referee_list_name()
{
    /* 查询 */
    $referee_list = referee_list_info();
    ...
    if (count($referee_list) > 0)
    {
        ...
        {
            $referee_name[$referee['user_id']] = $referee['real_name'];
        }
    }
