...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('integral_ranking_new_list.htm');
}
...
//        /* 过滤信息 */
        ...
        if($rank_level==3){
            //查出她下面的所有10万服务商
            if($account_type==2){
                $sql = "SELECT user_id FROM xm_integration_rank WHERE top_user_id={$top_user_id} AND create_time>{$time}";
        ...
        $table = $GLOBALS['ecs']->table('integration_rank') ." AS r ON u.user_id=r.user_id ";
        /* 记录总数 */
