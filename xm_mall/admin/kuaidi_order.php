...
if ($_REQUEST['act'] == 'set_postman_save')
{
	...
	$db->query($sql);
	$sql2 = "update ". $ecs->table('kuaidi_order_status')." set status_time='".time()."' where order_id='$order_id' AND status_id=2 ";
	$db->query($sql2);	
...
if ($_REQUEST['act'] == 'update')
{
	...
	$finish_time = $_POST['order_status'] =='4' ? time() : '0';
...
function set_order_status($o_id, $o_status)
{
		...
		if ($o_type == '0' || $o_type == '1')
		{
				...
				$sql = "update ". $GLOBALS['ecs']->table('kuaidi_order_status')." set status_time='".time()."' where  status_id = '$i' and order_id='$o_id' ";
				$GLOBALS['db']->query($sql);
		...
		{				
			$sql = "update ". $GLOBALS['ecs']->table('kuaidi_order_status')." set  status_display='1',status_time='".time()."'  where status_id = '7' and order_id='$o_id' ";
			$GLOBALS['db']->query($sql);				
		}
}
?>