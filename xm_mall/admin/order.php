...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    switch ($order_type) {
        case 1:
            $smarty->assign('ur_here', $_LANG['big_order_query']);
            $smarty->assign('action_link', array('href' => 'order.php?act=big_list', 'text' => $_LANG['02_order_list']));
        break;
        
        case 2:
            $smarty-> assign('ur_here', $_LANG['mid_order_query']);
            $smarty->assign('action_link', array('href' => 'order.php?act=mid_list', 'text' => $_LANG['03_order_list']));
        break;
        case 0:
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $order_list = order_list($_REQUEST['order_type']);//所有类型订单
    // 收货人电话处理
    foreach ($order_list['orders'] as $key => $value) {
        foreach ($value as $k => $v) {
            if($k == 'mobile') {
                $order_list['orders'][$key]['mobile'] = empty($v) ? $order_list['orders'][$key]['tel'] : $v;
            }
        }
    }
...
/*add 易业商城订单 2017年3月2日 by fzp*/
/*------------------------------------------------------ */
//-- 易业商城订单列表
/*------------------------------------------------------ */

elseif ($_REQUEST['act'] == 'big_list')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['02_order_list']);
    $smarty->assign('action_link', array('href' => 'order.php?act=order_query&order_type=1', 'text' => $_LANG['big_order_query']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('big_order_list.htm');
}
/*end fzp*/

/*add 精品商城订单 2017年3月2日 by fzp*/
/*------------------------------------------------------ */
//-- 中易物订单列表
/*------------------------------------------------------ */

elseif ($_REQUEST['act'] == 'mid_list')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    ...
    $order_list = order_list(2);//精品商城订单类型
    // 收货人电话处理
    foreach ($order_list['orders'] as $key => $value) {
        foreach ($value as $k => $v) {
            if($k == 'mobile') {
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('mid_order_list.htm');
}
/*end fzp*/

/*add H单订单 2017年8月15日 by liuhuaxia*/
/*------------------------------------------------------ */
//-- H单订单列表
/*------------------------------------------------------ */

elseif ($_REQUEST['act'] == 'h_list')
{
    /* 检查权限 */
    admin_priv('order_view');
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['04_order_list']);
    $smarty->assign('action_link', array('href' => 'order.php?act=order_query&order_type=4', 'text' => $_LANG['h_order_query']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('h_order_list.htm');
}
/*end liuhuaxia*/
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $order_type =  $_POST['order_type'];//获取订单类型
    ...
    if (isset($_REQUEST['order_id']))
    {
		...
		while($row_status = $db->fetchRow($res_status))
		{
			...
			{				
				...
				{
					case 1 :
						$shipping_info .= "您提交了订单，请等待确认。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 2 :
						$shipping_info .= "您的快件已经确认，等待快递员揽收。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 3 :
						...
						$shipping_info .= "您的快件正在派送，快递员：".$postman_info['postman_name']."，电话：".$postman_info['mobile']." (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 4 :
						$shipping_info .= "您的快件已经签收。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 5 :
						$shipping_info .= "您的快件已被拒收。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 6 :
						$shipping_info .= "您拒收的快件已被退回。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
					case 7 :
						$shipping_info .= "您的快件已经取消。 (".date('Y-m-d H:i:s', $row_status['status_time']).")";
						break;
				}
...
/*    echo '<pre>';
    ...
    //add by ymm 2017-2-23 10:17
    // ---------------------------查询order_info
    $order_info = "SELECT O.barter_type,O.cash_money_all,O.consume_money_all,O.shopping_money_all FROM  ". $ecs->table('order_info') ." AS O WHERE order_id =".$order[order_id];
    $order_infos = $db->getRow($order_info);

    /*add by ymm 2017-2-23 14:16*/
    /*--------------传递info数据到视图*/
    $smarty->assign('order_infos',array($order_infos['shopping_money_all'],$order_infos['consume_money_all'],$order_infos['cash_money_all']));
    /*--end */
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $shipping_status = ($order_finish == 1) ? SS_SHIPPED : SS_SHIPPED;
                ...
                if ($order['order_amount'] > 0)
                {
                    ...
                    if ($order['order_amount'] > 0)
                    {
                        ...
                        if (isset($_POST['surplus']) && floatval($_POST['surplus']) >= 0)
                        {
                            ...
                            /* 如果红包和积分和优惠券足以支付，把待付款金额改为0，退回部分积分优惠券 */
...
function export_invoice_to_excel($rows)
{
    ...
    if(is_array($rows) && count($rows) > 0)
    {
        ...
        if (EC_CHARSET != 'gbk')
        {
            echo iconv(EC_CHARSET, 'gbk//TRANSLIT//IGNORE', $data) . "\t";
        }
        else
        {
            echo $data. "\t";
...
/**
 * 退回优惠券、积分、红包（取消、无效、退货时），把订单使用优惠券、积分、红包设为0
...
function order_list($order_type)
{
    ...
    if ($result === false)
    {
        ...
        $filter['add_time'] = empty($_REQUEST['add_time']) ? '' : (strpos($_REQUEST['add_time'], '-') > 0 ?  strtotime($_REQUEST['add_time']) : $_REQUEST['add_time']);
		...
		$where = 'WHERE 1=1 ';
//        $where .= ($filter['supp']>0) ? 'AND o.supplier_id > 0' : 'AND o.supplier_id = 0';
        ...
        switch ($order_type) {
            case 0: //所有类型
               $where .= "AND 1=1";
                break;
             case 1: //易业商城订单
                $where .= " AND o.barter_type = 1";
                break;
            case 2: //精品商城订单
                $where .= " AND o.barter_type = 2";
                break;
            case 4:
                $where .= " AND o.barter_type = 4";
                break;
        }
        ...
        /* 查询 */
        //edit ymm 2017-2-22 17:18
        /* --------新增几个查询字段 barter_type  shopping_money_all consume_money_all cash_money_all     */
        if($filter['supp']){
        	$sql = "SELECT o.order_id, o.order_sn, o.add_time,o.barter_type,o.shopping_money_all,o.consume_money_all,o.cash_money_all, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
        	...
        	$sql = "SELECT o.order_id, o.order_sn,o.barter_type,o.shopping_money_all,o.consume_money_all,o.cash_money_all, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        /*增值税发票_添加_START_www.68ecshop.com*/
        $row[$key]['formatted_add_time'] = date('Y-m-d H:i',$value['add_time']);
...
function delivery_list()
{
    ...
    foreach ($row AS $key => $value)
    {
        $row[$key]['add_time'] = date($GLOBALS['_CFG']['time_format'], $value['add_time']);
        $row[$key]['update_time'] = date($GLOBALS['_CFG']['time_format'], $value['update_time']);
        if ($value['status'] == 1)
        {
...
function back_list()
{
    ...
    foreach ($row AS $key => $value)
    {
        $row[$key]['return_time'] = date($GLOBALS['_CFG']['time_format'], $value['return_time']);
        $row[$key]['add_time'] = date($GLOBALS['_CFG']['time_format'], $value['add_time']);
        $row[$key]['update_time'] = date($GLOBALS['_CFG']['time_format'], $value['update_time']);
        if ($value['status'] == 1)
        {
...
function new_kuaidi_order($delivery_id)
{
	...
	$sql = "insert into ".$GLOBALS['ecs']->table('kuaidi_order')." (user_id, to_name, to_tel, to_region_id, to_address, goods_name, package_num, start_time, end_time, add_time) values ('$delivery_order[user_id]', '$delivery_order[consignee]', '$to_tel', '$delivery_order[district]', '$delivery_order[address]', '$delivery_id', '1', '0', '0', '".time()."')";
	$GLOBALS['db']->query($sql);
	...
	foreach ($orderstatus_array AS $okey=>$oval)
	{
		...
		$sql="insert into ".$GLOBALS['ecs']->table('kuaidi_order_status').
					...
					"values('$order_id', '$okey', '$oval[name]', '$oval[type]', '$status_display', '".time()."') ";
		$GLOBALS['db']->query($sql);
	}
}
