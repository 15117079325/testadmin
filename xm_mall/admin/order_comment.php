...
elseif ($_REQUEST['act'] == 'toggle_change')
{
    ...
    if ($GLOBALS['db']->query("UPDATE xm_comments SET cmt_status={$is_comment} WHERE cmt_id={$grade_id}"))
    {
        clear_cache_files();
        if($is_comment==2){
           $is_comment =0;
    ...
    else
    {
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
               ...
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$_POST[content]','" . time() . "', '$ip', '0', '$_POST[comment_id]')";
    }
    $db->query($sql);
...
function get_order_comment_list()
{
    ...
    $filter['sort_by']      = empty($_REQUEST['sort_by']) ? 'cmt_gmt_create' : trim($_REQUEST['sort_by']);
    $filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

    $where = " WHERE 1 ";
    if(!empty($filter['keywords'])){
        $order_id = $GLOBALS['db']->getOne("SELECT order_id FROM xm_orders WHERE order_sn='{$filter['keywords']}'");
        $where .= " AND order_id={$order_id} ";
    }

	
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comments'). "  $where ";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comments'). " $where " .
    ...
    {
		$row['add_time'] = date("Y-m-d H:i:s",$row['cmt_gmt_create']);
		$row['order_sn'] = $GLOBALS['db']->getOne("SELECT order_sn FROM xm_orders WHERE order_id={$row['order_id']}");
		$row['user_name'] = $GLOBALS['db']->getOne("SELECT user_name FROM xm_users WHERE user_id={$row['user_id']}");
        $arr[] = $row;
    }
