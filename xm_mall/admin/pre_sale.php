...
function action_insert_update ()
{
	...
	if(isset($_POST['finish']))
	{
		...
		/* 结束预售活动，修改结束时间为当前时间 */
		$sql = "UPDATE " . $ecs->table('goods_activity') . " SET end_time = '" . time() . "' " . "WHERE act_id = '$pre_sale_id' LIMIT 1";
		$db->query($sql);
...
{
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE goods_id = '$goods_id' " . " AND act_type = '" . GAT_PRE_SALE . "'" . " AND start_time <= " . time() . " AND end_time >= " . time();
