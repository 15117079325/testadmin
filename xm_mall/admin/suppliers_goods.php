...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
        ...
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . time() . "', '". time() ."', '$goods_type', '$rank_integral')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . time() . "', '". time() ."', '$goods_type', '$code', '$rank_integral')";
        }
    }
    else
    {
    ...
    /* 记录上一次选择的分类和品牌 */
    setcookie('ECSCP[last_choose]', $catgory_id . '|' . $brand_id, time() + 86400);
    /* 清空缓存 */
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_name')
{
    ...
    if ($exc->edit("goods_name = '$goods_name', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
    ...
    {
        if ($exc->edit("shop_price = '$goods_price', market_price = '$price_rate', last_update=" .time(), $goods_id))
        {
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
    ...
    if ($exc->edit("goods_number = '$goods_num', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'toggle_on_sale')
{
    ...
    if ($exc->edit("is_on_sale = '$on_sale', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'toggle_best')
{
    ...
    if ($exc->edit("is_best = '$is_best', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'toggle_new')
{
    ...
    if ($exc->edit("is_new = '$is_new', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'toggle_hot')
{
    ...
    if ($exc->edit("is_hot = '$is_hot', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
    ...
    $exc->edit("is_delete = 0, add_time = '" . time() . "'", $goods_id);
    clear_cache_files();
