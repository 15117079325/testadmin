...
//var_dump($temp_libs);die();
    foreach ($temp_libs AS $val)
    {
        ...
        elseif ($val['lib'] == 'barter_goods')
        {
            /* 分类下的商品 */
            if (isset($db_dyna_libs[$val['region']][$val['library']]) && ($row = array_shift($db_dyna_libs[$val['region']][$val['library']])))
            {
                $barter_goods[] = array('region' => $val['region'], 'sort_order' => $val['sort_order'], 'number' => $row['number'], 'barter_type' => $row['id'], 'ext_info'=>$row['ext_info']);
                //var_dump( $barter_goods);die();
            }
            else
            {
                $barter_goods[] = array('region' => $val['region'], 'sort_order' => $val['sort_order'], 'number'=>0, 'barter_type' => 0,'ext_info'=>array());
            }
        }
   ...
   //$smarty->assign('cate_goods',         $cate_goods);
    $smarty->assign('barter_goods',         $barter_goods);
    ...
    $smarty->assign('arr_barter_types',         get_barter_type_list());
...
if ($_REQUEST['act'] == 'setting')
{
    ...
    /* 易物的商品 */
    if (isset($_POST['regions']['barter_goods']))
    {
        foreach ($_POST['regions']['barter_goods'] AS $key => $val)
        {

            if ($_POST['barter_types']['barter_goods'][$key] != '' && intval($_POST['barter_types']['barter_goods'][$key]) > 0)
            {
                $template = array(
                    // 主题
                    'theme' => $curr_template,
                    ...
                    'region' => $val,
                    // 库文件
                    'library' => '/library/barter_goods.lbi',
                    // 排序
                    'sort_order' => $_POST['sort_order']['barter_goods'][$key],
                    //
                    'type' => 9,
                    // 分类ID
                    'id' => $_POST['barter_types']['barter_goods'][$key],
                    // 数量
                    'number' => $_POST['number']['barter_goods'][$key],
                    // 扩展信息
                    'ext_info' => serialize(array(
                        //简称
                        'short_name' => empty($_POST['ext_info']['short_name'][$key]) ? '' : $_POST['ext_info']['short_name'][$key],
                        //主题
                        'barter_color' => empty($_POST['ext_info']['barter_color'][$key]) ? '' : $_POST['ext_info']['barter_color'][$key]
                    ))
                );
    ...
    foreach ($_POST['regions'] AS $key => $val)
    {
        switch ($key)
        {
            ...
            case 'barter_goods':
                foreach ($val AS $k => $v)
                {
                    if (intval($_POST['barter_types']['barter_goods'][$k]) > 0)
                    {
                        $post_regions[] = array('region'     => $v,
                            'type'       => 1,
                            'number'     => $_POST['number']['barter_goods'][$k],
                            'library'    => '/library/' .$key. '.lbi',
                            'sort_order' => $_POST['sort_order']['barter_goods'][$k],
                            'id'         => $_POST['barter_types']['barter_goods'][$k]);
                    }
                }
                break;
            case 'brand_goods':
    ...
    /* 排序 */
    usort($post_regions, "array_sort_cmp");
