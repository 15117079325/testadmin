...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    header('Content-Disposition:inline;filename="' . urlencode($outputFileName) . '"');
...
function get_result_list ($area_type, $province, $city)
{
    ...
    if($result === false)
    {
        ...
        if($filter['area_type'] == 0)
        {
            $sql = "SELECT COUNT(*) order_count,sum(if( o.barter_type=1 ,1,0)) yy_count , sum(if( o.barter_type=2 ,1,0)) jp_count, SUM(o.shopping_money_all + o.cash_money_all) goods_amount,
                sum(case o.barter_type when 1 then o.shopping_money_all else 0 end) as yy_gcount,
                sum(case o.barter_type when 2 then o.cash_money_all else 0 end) as jp_gcount,
                o.province, 0 city, 0 district, (SELECT r.region_name FROM "
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.province) province_name FROM '
                . $GLOBALS['ecs']->table('order_info') . ' o ' . $where . ' GROUP BY o.province ORDER BY order_count DESC';
        }
        // 按市统计
        elseif($filter['area_type'] == 1)
        {
            $sql = "SELECT COUNT(*) order_count,sum(if( o.barter_type=1 ,1,0)) yy_count , sum(if( o.barter_type=2 ,1,0)) jp_count, SUM(o.shopping_money_all +  o.cash_money_all) goods_amount,
                sum(case o.barter_type when 1 then o.shopping_money_all else 0 end) as yy_gcount,
                sum(case o.barter_type when 2 then o.cash_money_all else 0 end) as jp_gcount,o.province, o.city, 0 district, (SELECT r.region_name FROM "
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.province) province_name, (SELECT r.region_name FROM '
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.city) city_name FROM '
                . $GLOBALS['ecs']->table('order_info') . ' o ' . $where
        ...
        else
        {
            $sql = 'SELECT COUNT(*) order_count, sum(if( o.barter_type=1 ,1,0)) yy_count , sum(if( o.barter_type=2 ,1,0)) jp_count, SUM(o.shopping_money_all + o.cash_money_all) goods_amount,
                sum(case o.barter_type when 1 then o.shopping_money_all else 0 end) as yy_gcount,
                sum(case o.barter_type when 2 then o.cash_money_all else 0 end) as jp_gcount,
                o.province, o.city, o.district, (SELECT r.region_name FROM '
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.province) province_name, (SELECT r.region_name FROM '
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.city) city_name, (SELECT r.region_name FROM '
                . $GLOBALS['ecs']->table('region') . ' r WHERE r.region_id = o.district) district_name FROM '
                . $GLOBALS['ecs']->table('order_info') . ' o ' . $where
