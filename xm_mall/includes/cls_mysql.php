...
class cls_mysql
{
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        else
        {
            if (PHP_VERSION >= '4.2')
            {
                $this->link_id = @mysqli_connect($dbhost, $dbuser, $dbpw, $dbname,3306,'');
            }
            else
            {
                $this->link_id = @mysqli_connect($dbhost, $dbuser, $dbpw, $dbname,3306,'');
    ...
    function query($sql, $type = '')
    {
        ...
        if (!($query = mysqli_query( $this->link_id,$sql)) && $type != 'SILENT')
        {
            ...
            $this->error_message[]['SQL'] = $sql;
            $this->error_message[]['错误'] = mysqli_error($this->link_id);
            $this->error_message[]['代码'] = mysqli_errno($this->link_id);
