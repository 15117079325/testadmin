...
//require(ROOT_PATH . 'includes/lib_log.php');
//$handler = new CLogFileHandler($path,$fileName);
//Log::Init($handler);
...
//require_once ROOT_PATH . '/maqu/lumen.php';
...
if (!defined('INIT_NO_USERS'))
{
    ...
    if (empty($_SESSION['user_id']))
    {
        if ($user->get_cookie())
        {
            /* 如果会员已经登录并且还没有获得会员的帐户优惠券、积分以及优惠券 */
...
/* 判断是否有其他个用户登录，如果有其他用户登录，就把前一个用户挤掉 */
function checkSingleLogin(){
    /* 说明有其他用户登录*/
    /* 先cookie验证 */
    if(isset($_COOKIE['access_token'])){
        $access_token=$_COOKIE['access_token'];
        $sql = "select * from" .$GLOBALS['ecs']->table('users_safe') ." where access_token ="."'$access_token'";
        $row = $GLOBALS['db']->getRow($sql);
        if($row){
            //客户机 token验证正确, 用session在验证
            if(isset($_SESSION['access_token'])){
                ...
                if($row){
                    ...
                    //如果不存在。就说明这个access_token已经被其他登陆者更新过了,跳转到登录页面
                    return false;
                }
            }
            return true;
        }else{
            //如果不存在。就说明这个access_token已经被其他登陆者更新过了
            return false;
        }
    }
    /* $_COOKIE['access_token']在客户机被删除 ，直接跳转到登录页面 */
    if(empty($_COOKIE['access_token'])&&isset($_SESSION['access_token'])){
       return false;
    }
