...
{
    $sql = 'SELECT  x.cash_credit, x.shopping_credit, x.consume_credit, x.belongto as barter_type, g.goods_id, g.goods_name, g.goods_thumb, g.market_price, g.shop_price AS org_price, '.
        ...
        " LEFT JOIN " . $GLOBALS['ecs']->table('mq_goods_extra') . " AS x ".
        "ON x.goods_id = c.goods_id ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //add 个人收藏商品积分展示 2017年3月1日17:05:54
        $goods_list[$row['goods_id']]['cash_credit']    = $row['cash_credit'];
        $goods_list[$row['goods_id']]['consume_credit']    = $row['consume_credit'];
        $goods_list[$row['goods_id']]['shopping_credit']    = $row['shopping_credit'];
        $goods_list[$row['goods_id']]['barter_type']    = $row['barter_type'];
        $goods_list[$row['goods_id']]['goods_url'] = build_uri('barter_goods', array('gid'=>$row['goods_id']), $row['goods_name']);
        //end
        $goods_list[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
        //var_dump( $goods_list[$row['goods_id']]['goods_url']);die();
 ...
 *
 * @access  public
 * @param   array     $surplus  会员优惠券信息
 * @param   string    $amount   优惠券
 *
 * @return  int
 */
 ...
 *
 * @access  public
 * @param   array     $surplus_id  优惠券记录的ID
 ...
 * @param   bool    $include_balance    是否包含优惠券支付（冲值时不应包括）
 ...
 */
// edit sql语句中加入查询7个积分信息 2017年2月24日 10:04:34 by fzp
...
/*代码修改2014-12-23 by www.68ecshop.com  _star */
    ...
    $sql = "SELECT u.*, ma1.money as cash_credit, ma2.money as consume_credit, ma3.money as invest_credit, ma4.money as register_credit, ma5.money as share_credit, ma6.money as shopping_credit, ma7.money as useable_credit " .
...
/*代码修改2014-12-23 by www.68ecshop.com  _end */
    ...
    // add 新增的7条积分信息 2017年2月24日 10:07:44 by fzp
    $info['cash_credit'] = $row['cash_credit'];
    $info['consume_credit'] = $row['consume_credit'];
    $info['invest_credit'] = $row['invest_credit'];
    $info['register_credit'] = $row['register_credit'];
    $info['share_credit'] = $row['share_credit'];
    $info['shopping_credit'] = $row['shopping_credit'];
	$info['useable_credit'] = $row['useable_credit'];
	...
	$info['status'] = $row['status'];
    $info['refusal_reason'] = $row['refusal_reason'];
    $info['hand_photo'] = $row['hand_photo'];
    $info['edit_password'] = $row['edit_password'];
    ...
    if ($last_time == 0)
    {
        $_SESSION['last_time'] = $last_time = time();
    }
...
/**
 * add 取得用户服务中心等级信息 2017年2月28日 20:47:56
 * @access   public
 * @author   fzp
 *
 * @return array
 */
function get_rank_info_mq()
{
    global $db,$ecs;
    if (!empty($_SESSION['user_rank']))
    {
 ...
 *
 */
//edit by fei 2017年2月27日16:56:15 个人中心页面我的购物车
...
//	$sql = "SELECT c.goods_price,c.goods_id,g.* FROM ".$GLOBALS['ecs']->table('goods')."as g,".$GLOBALS['ecs']->table('cart')."as c WHERE c.user_id='$user_id' AND g.goods_id=c.goods_id";
    $sql = "SELECT c.shopping_money,c.consume_money,c.cash_money,c.barter_type,c.goods_id,g.* FROM ".$GLOBALS['ecs']->table('goods')."as g,".$GLOBALS['ecs']->table('cart')."as c WHERE c.user_id='$user_id' AND g.goods_id=c.goods_id";
