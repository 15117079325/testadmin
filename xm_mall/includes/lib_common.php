...
/**
 * 验证输入的积分是否合法
 * add 2017年2月22日 17:05:02 by fzp
 * @access  public
 * @param   string      $user_credit      验证需要的数值
 *
 * @return bool
 */

function is_num($user_credit)
{
    $chars = "/^[1-9]\d*|0$/";
    if(preg_match($chars, $user_credit))
    {
        return true;
    }
...
/**
 * 取得易物类型
 * @return array 易物类型 id => name
 */
// 代码修改_start_derek20150129admin_goods  www.68ecshop.com
function get_barter_type_list()
// 代码修改_end_derek20150129admin_goods  www.68ecshop.com
{
    $barter_type_list[1] = addslashes('易业商城');
    $barter_type_list[2] = addslashes('精品商城');
    $barter_type_list[3] = addslashes('小易物');

    return $barter_type_list;
}
...
//                    }
        ...
        case 'exchange':
            ...
            break;
        // edit by fei 2017年2月21日15:52:09 易业商城首页 分页引用
        case 'barter_big':
            if ($rewrite)
            {
                $uri = 'barter_big-' . $cat_id;
                if (isset($price_min))
                {
                    $uri .= '-min'.$price_min;
                }
            ...
            {
                $uri = 'barter_big.php?cat_id=' . $cat_id;
                if (isset($price_min))
                {
                    $uri .= '&amp;integral_min=' . $price_min;
                }
            ...
            break;
        //end
        // edit by fei 2017年2月21日15:52:09 大易物商城首页 分页引用
        case 'barter_mid':
            if ($rewrite)
            {
                $uri = 'barter_big-' . $cat_id;
            ...
            {
                $uri = 'barter_mid.php?cat_id=' . $cat_id;
                if (isset($price_min))
                {
                    $uri .= '&amp;integral_min=' . $price_min;
                }
        ...
        break;
		      // edit by liuhuaxia 2017年2月21日15:52:09 H单商城首页 分页引用
        case 'barter_h':
            if ($rewrite)
            {
                $uri = 'barter_h-' . $cat_id;
            ...
            {
                $uri = 'barter_h.php?cat_id=' . $cat_id;
                if (isset($price_min))
                {
                    $uri .= '&amp;integral_min=' . $price_min;
                }
        ...
        case 'exchange_goods':
            ...
            break;
        //edit shengl 二〇一七年二月二十七日 01:15:41 商品详情统一页面
        case 'barter_goods':
            if (empty($gid))
            {
                return false;
...
// edit 更新记录账户7个积分 变动 2017年2月27日 13:56:23 by fzp
/**
 * 记录帐户变动
 * @param   int     $user_id        用户id
 * @param   float   $user_money     可用优惠券变动
 * @param   float   $frozen_money   冻结优惠券变动
 ...
 * 修改后
 */
function log_account_change_mq($user_id, $credit, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    // $credit['cash'] = $cash;
    // $credit['consume'] = $consume;
    // $credit['invest'] = $invest;
    // $credit['register'] = $register;
    // $credit['share'] = $share;
    // $credit['shopping'] = $shopping;
    // $credit['useable'] = $useable;
// foreach ($credit as $key => $value) {
//     var_dump($value['account_type']);
// }
    // 写入日志
    $change_time = time();
    $sql = "SELECT mue.account_id FROM " . $GLOBALS['ecs']->table('mq_users_extra') . " AS mue LEFT JOIN " . 
            $GLOBALS['ecs']->table('users') . " AS u ON mue.user_id = u.user_id WHERE u.user_id =" . $user_id;
    $account_id = $GLOBALS['db']->getOne($sql);
    foreach ($credit as $value) {
       $sql = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account_log') . " (`account_type`, `change_money`, `income_type`, `money`, `change_time`, `change_desc`, `change_type`, `account_id`) ".
            " VALUES ('" . $value['account_type'] . "','" . $value['change_money'] . "','" . $value['income_type'] . "','" .
            $value['money'] . "','" . "$change_time', '$change_desc', '$change_type', '$account_id') " ;
            $GLOBALS['db']->query($sql);
    }
    /* 更新用户信息 */
    foreach ($credit as $value) {
        $sql = "UPDATE" . $GLOBALS['ecs']->table('mq_account'). " SET money = ". $value['money']. ", update_at = '$change_time'" . 
            " WHERE account_id = ". "'$account_id'". " AND account_type = "."'".$value['account_type']."'". " LIMIT 1";
        $GLOBALS['db']->query($sql);
    }
}
// end
...
function get_image_path($goods_id, $image='', $thumb=false, $call='goods', $del=false)
{
    ...
    //处理类偶尔无法加载
    if(class_exists('\maqu\filesystem\FileSystemManager')){
        $url = empty($image) ? $GLOBALS['_CFG']['no_picture'] : \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($image);
    }else{
        $url = $image;
        if(empty($url)){
            $url =  $GLOBALS['_CFG']['no_picture'];
        }else{
            $url_head = substr(trim($image),0,4);
            if($url_head !='http'){
                $url =STATIC_RESOURCE_URL.$image;
            }
        }
    }
    return $url;
...
{ 
	if(!$text) return '';
	if(EC_CHARSET != 'gbk') $text = iconv(EC_CHARSET, 'gbk//TRANSLIT//IGNORE', $text);
	$data = array();
...
function Recordkeyword($word_www_68ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	...
	if($r)
	{
		...
		$month_search = date('Y-m', $r['updatetime']) == date('Y-m', time()) ? 'month_search+1' : '1';
		$week_search = date('W', $r['updatetime']) == date('W', time()) ? 'week_search+1' : '1';
		$today_search = date('Y-m-d', $r['updatetime']) == date('Y-m-d', time()) ? 'today_search+1' : '1';
	...
	else
	{
		...
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_www_68ecshop_com', '$word_www_68ecshop_com', '$letter_www_68ecshop_com', '$items', '".time()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_www_68ecshop_com);
	...
	if (!empty($w_id))
	{
    	...
    	$sql_www_68ecshop_com = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                ...
                "'$w_id', '".time()."', '$ip_www_68ecshop_com', '$area_www_68ecshop_com')";
    	$GLOBALS['db']->query($sql_www_68ecshop_com);
	}
}
/* 代码增加_END    by   www.68ecshop.com */
 ...
 * @return  商品最终购买价格
 */
function get_barter_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    $final_price   = [
        'shopping_credit'=>0,
        'cash_credit'=>0,
    ...
    //取得商品促销价格列表
    /* 取得商品信息 */
    $sql = "SELECT mge.shopping_credit,mge.cash_credit,mge.consume_credit,mge.allow_bb,mge.bb_percent,mge.bb_life_days,mge.bb_daily_buy_limit ".
        " FROM " .$GLOBALS['ecs']->table('goods'). " AS g ".
        " LEFT JOIN " . $GLOBALS['ecs']->table('mq_goods_extra') . " AS mge ".
        "ON g.goods_id = mge.goods_id ".
        " WHERE g.goods_id = '" . $goods_id . "'" .
        " AND g.is_delete = 0";
    $goods = $GLOBALS['db']->getRow($sql);

    $final_price['shopping_credit'] += $goods['shopping_credit'];
    $final_price['cash_credit'] += $goods['cash_credit'];
    $final_price['consume_credit'] += $goods['consume_credit'];
    $final_price['bb_percent_price'] += ($goods['consume_credit']+ $goods['cash_credit'])*$goods['bb_percent']/100;
    ...
    if ($is_spec_price)
    {
        if (!empty($spec))
        {
            $spec_price   = barter_spec_price($spec);
//            $final_price['shopping_credit'] += $spec_price['shopping_credit'];
//            $final_price['cash_credit'] += $spec_price['cash_credit'];
//            $final_price['consume_credit'] += $spec_price['consume_credit'];
//            $final_price['bb_percent_price'] += ($spec_price['consume_credit']+ $spec_price['cash_credit'])*$goods['bb_percent']/100;
            $final_price['shopping_credit'] = $spec_price['shopping_credit'];
            $final_price['cash_credit'] = $spec_price['cash_credit'];
            $final_price['consume_credit'] = $spec_price['consume_credit'];
            $final_price['bb_percent_price'] = ($spec_price['consume_credit']+ $spec_price['cash_credit'])*$goods['bb_percent']/100;
 ...
 *
 */
function check_activate($user_id){

    $sql="SELECT user_cx_rank FROM". $GLOBALS['ecs']->table('mq_users_extra')."WHERE user_id=".$user_id;
    $user_cx_rank=$GLOBALS['db']->getOne($sql);
    return $user_cx_rank;
}
/**
 * 验证用户是否实名认证
 *
 */
function check_certification($user_id){

    //判断实名认证状态
    $sql="SELECT status FROM". $GLOBALS['ecs']->table('users')."WHERE user_id=".$user_id;
    $status = $GLOBALS['db']->getOne($sql);
    return $status;
}

/*查询用优惠券*/
function check_user_money($user_id)
{
    //判断用户优惠券是否可以申购
    $sql_wherer = "b.user_id='". $user_id ."' ";

    $user_money = "SELECT IFNULL(ma1.money,0) as cash_credit, IFNULL(ma2.money,0) as consume_credit ".
