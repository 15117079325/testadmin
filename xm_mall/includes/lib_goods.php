...
function get_top10($cats = '')
{
   ...
   $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number, mge.cash_credit,
mge.consume_credit, mge.shopping_credit, mge.belongto ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g, ' .
                $GLOBALS['ecs']->table('order_info') . ' AS o, ' .
                $GLOBALS['ecs']->table('order_goods') . ' AS og, ' .
                $GLOBALS['ecs']->table('mq_goods_extra') . ' AS mge ' .
           " WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.goods_id = mge.goods_id $where $top10_time ";
    //判断是否启用库存，库存数量是否大于0
    ...
    $sql .= ' AND og.order_id = o.order_id AND og.goods_id = g.goods_id ' .
           ...
           " AND (mge.belongto = 1 OR mge.belongto = 2) ". //筛选大中易物
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.goods_brief , g.market_price, g.shop_price AS org_price, g.promote_price, mge.shopping_credit, mge.cash_credit, mge.consume_credit, mge.belongto, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                " LEFT JOIN ".$GLOBALS['ecs']->table('mq_goods_extra')." AS mge ON g.goods_id = mge.goods_id " .
        ...
        $sql .= ' AND (mge.belongto = 1 OR mge.belongto = 2) '; //筛选大中易物
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['shopping_credit']   = $row['shopping_credit'];
            $goods[$idx]['cash_credit']   = $row['cash_credit'];
            $goods[$idx]['consume_credit']   = $row['consume_credit'];
            $goods[$idx]['belongto']   = $row['belongto'];
            // end fzp
...
function get_promote_goods($cats = '')
{
    ...
    foreach ($result AS $idx => $row)
    {
		...
		$goods[$idx]['lefttime']     = $row['promote_end_date'] - time();
        $goods[$idx]['brief']        = $row['goods_brief'];
...
{
    $time = time();
//    $sql = 'SELECT g.*, c.measure_unit, mq.cash_credit, mq.consume_credit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
...
//        "GROUP BY g.goods_id";
    //edit 2017年2月22日16:54:54  关联mq_goods_extra 添加中易物 积分信息
    $sql = 'SELECT g.*, c.measure_unit, mq.belongto, mq.shopping_credit, mq.cash_credit, mq.consume_credit, mq.allow_bb,mq.bb_percent,mq.bb_life_days,mq.bb_daily_buy_limit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
        'IFNULL(AVG(r.comment_rank), 0) AS comment_rank, ' .
        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price " .
        'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        'LEFT JOIN ' . $GLOBALS['ecs']->table('category') . ' AS c ON g.cat_id = c.cat_id ' .
...
//END
    ...
    if ($row !== false)
    {
        ...
        $row['bb_percent_price'] += ($row['consume_credit']+ $row['cash_credit'])*$row['bb_percent']/100;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.attr_price, g.attr_shopping_credit," .
                "g.attr_cash_credit,g.attr_consume_credit,mqg.belongto ".
            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
           'LEFT JOIN ' . $GLOBALS['ecs']->table('mq_goods_extra') . ' AS mqg ON mqg.goods_id = g.goods_id ' .
    ...
    foreach ($res AS $row)
    {
        ...
        if( $row['belongto']==1){
            $fomartprice="购物券:".$row['attr_shopping_credit'];
        }elseif($row['belongto']==2){
            $fomartprice="新美积分:".$row['attr_cash_credit'] ."+ 消费积分:".$row['attr_consume_credit'];
        }else{
            $fomartprice=price_format(abs($row['attr_price']), false);
        }
        if ($row['attr_type'] == 0)
        {
 ...
 *
 * @access  public
 * @param   integer     $barter_type_id     易物分类ID
 * @param   integer     $num        数量
 * @param   string      $from       来自web/wap的调用
 * @param   string      $order_rule 指定商品排序规则
 * @return  array
 */
function assign_barter_goods($barter_type_id, $num = 0, $from = 'web', $order_rule = '')
{
    $sql = 'SELECT g.goods_id, g.goods_name, mge.shopping_credit,mge.cash_credit,mge.consume_credit, ' .
        'g.goods_brief, g.goods_thumb, g.goods_img ' .
        "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
        "LEFT JOIN " . $GLOBALS['ecs']->table('mq_goods_extra') . ' AS mge '.
        "ON mge.goods_id = g.goods_id ".
        'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
        'g.is_delete = 0 AND mge.belongto = '.$barter_type_id . ' ';
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('barter_goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $goods[$idx]['shopping_credit'] = $row['shopping_credit'];
        $goods[$idx]['cash_credit'] = $row['cash_credit'];
        $goods[$idx]['consume_credit'] = $row['consume_credit'];
    }
    ...
    if ($from == 'web')
    {
        $GLOBALS['smarty']->assign('barter_goods_' . $barter_type_id, $goods);
    }
    elseif ($from == 'wap')
    {
        $barter['goods'] = $goods;
    ...
    /* 分类信息 */
    switch($barter_type_id){
        case '1':
            $barter['name'] = '易业商城';
            $barter['url']  = build_uri('barter_big', array('id' => $barter_type_id));
            $barter['id']   = $barter_type_id;;
        case '2':
            $barter['name'] = '精品商城';
            $barter['url']  = build_uri('barter_mid', array('id' => $barter_type_id));
            $barter['id']   = $barter_type_id;;
        case '3':
            $barter['name'] = '小易物';
            $barter['url']  = build_uri('barter_small', array('id' => $barter_type_id));
            $barter['id']   = $barter_type_id;;
    }
 ...
 * @return  void
 */
function barter_spec_price($spec)
{
    if (!empty($spec))
    {
        if(is_array($spec))
        ...
        $sql = 'SELECT SUM(attr_shopping_credit) AS shopping_credit,SUM(attr_cash_credit) AS cash_credit,SUM(attr_consume_credit) AS consume_credit FROM ' . $GLOBALS['ecs']->table('goods_attr') . " WHERE $where";
