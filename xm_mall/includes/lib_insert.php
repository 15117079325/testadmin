...
function insert_query_info()
{
    ...
    /* 加入触发cron代码 */
    $cron_method = empty($GLOBALS['_CFG']['cron_method']) ? '<img src="api/cron.php?t=' . time() . '" alt="" style="width:0px;height:0px;" />' : '';
 ...
 * @return  string
 */
// edit  购物车添加 我看过的 商品价格
function insert_history()
{
    $str = '';
    if (!empty($_COOKIE['ECS']['history']))
    {
        $where = db_create_in($_COOKIE['ECS']['history'], 'a.goods_id');
        $sql   = 'SELECT a.goods_id, a.goods_name, a.goods_thumb, b.consume_credit, b.shopping_credit, b.cash_credit, b.belongto as barter_type FROM ' . $GLOBALS['ecs']->table('goods') .' AS a '.
                ' LEFT JOIN '.$GLOBALS['ecs']->table('mq_goods_extra').' AS b ON a.goods_id = b.goods_id '.   
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $goods['barter_type'] = $row['barter_type'];
            ...
            $goods['cash_credit'] = $row['cash_credit'];
            $goods['consume_credit'] = $row['consume_credit'];
            $goods['shopping_credit'] = $row['shopping_credit'];
            $goods['url'] = build_uri('barter_goods', array('gid'=>$row['goods_id']), $row['goods_name']);

		  
            $str.='<li style="height:136px;">
              ...
              <div class="p-name"><a target="_blank" href="'.$goods['url'].'">'.$goods['goods_name'].'</a> </div>';
                if($goods['barter_type'] == 2){
                    $str.='<div class="p-comm"><span class="p-price main-color"； style="text-align: right">¥ '.$goods['cash_credit'].'</span> </div>
                    <div class="p-comm"><span class="p-price main-color">消费积分:'.$goods['consume_credit'].'</span> </div>';
                }elseif($goods['barter_type'] == 1){
                    $str.='<div class="p-comm"><span class="p-price main-color">购物券:'.$goods['shopping_credit'].'</span> </div>';

                }else{
                    $str.='<div class="p-comm"><span class="p-price main-color">该商品已失效</span> </div>';
                }
            $str.= '</li>';
        }
...
//edit shengl 2017年2月25日19:56:16 修改数据内容
//$sql = "select * from ".$GLOBALS['ecs']->table('goods')." where cat_id = ".$catid." and is_best = 1 AND is_delete = 0";
$sql = "select g.*,mge.belongto,mge.shopping_credit,mge.cash_credit,mge.consume_credit from ".$GLOBALS['ecs']->table('goods')." AS g LEFT JOIN ".$GLOBALS['ecs']->table('mq_goods_extra')." AS mge ON g.goods_id = mge.goods_id where cat_id = ".$catid." and is_best = 1 AND is_delete = 0";
//end shengl
...
//edit shengl 2017年2月25日19:59:37 新增字段
$arr[$key]['barter_type'] = $rows['belongto'];
$arr[$key]['shopping_credit'] = $rows['shopping_credit'];
$arr[$key]['cash_credit'] = $rows['cash_credit'];
$arr[$key]['consume_credit'] = $rows['consume_credit'];
//end shengl
}
return $arr;
}
...
function insert_cart_info()
{
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0  ";
     //edit 2017年2月23日10:12:37 消费积分 新美积分
     ...
     //    " left join ".$GLOBALS['ecs']->table('mq_goods_extra')." as mq on mq.goods_id=c.goods_id ".
     //    " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "'";
   //END
    $sql = 'SELECT c.*,pa.*,IF(c.extension_code = "package_buy",act_name,g.goods_name) as goods_name, ' .
		' IF(c.extension_code = "package_buy","package_img",g.goods_thumb) as goods_thumb,g.goods_id,c.goods_number,c.cash_money,c.consume_money,c.barter_type,c.goods_price' .
		' FROM ' . $GLOBALS['ecs']->table('cart') ." AS c ".
		" LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id=c.goods_id ".
		" left join ".$GLOBALS['ecs']->table('goods_activity')." as pa on pa.act_id=c.goods_id ".
		" WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "' AND c.barter_type = 2 ";
    $row = $GLOBALS['db']->GetAll($sql);
		...
		foreach($row AS $k=>$v)
		{
                ...
                //add 2017年2月23日10:05:56 购物车中新美积分 消费积分
                $arr[$k]['cash_credit']  = $v['cash_money'];
                $arr[$k]['consume_credit']  = $v['consume_money'];
                $arr[$k]['belongto']     = $v['barter_type'];
    ...
    //        ' FROM ' . $GLOBALS['ecs']->table('cart') ." AS c ".
    //        " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "'";
    //edit 2017年2月23日10:17:26 购物车中商品积分使用
    $sql = 'SELECT SUM(goods_number) AS number, SUM(cash_money * goods_number) AS cash_credit, SUM(consume_money * goods_number) AS consume_credit' . 
           ' FROM ' . $GLOBALS['ecs']->table('cart') ." AS c ".
           " WHERE $sql_where AND rec_type = '" . CART_GENERAL_GOODS . "' AND c.barter_type = 2 ";
	...
	foreach($arr as $val){
		...
		$zj['cash_credit'] += $val['cash_credit']*$val['goods_number'];
        $zj['consume_credit'] += $val['consume_credit']*$val['goods_number'];
	}

    $GLOBALS['smarty']->assign('str',sprintf($GLOBALS['_LANG']['cart_info'], $number, price_format($cash_credit, false),price_format($consume_credit, false)));
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath(DATA_DIR . "/afficheimg/$row[ad_code]")  : $row['ad_code'];
