...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_id,u.user_money,u.email,u.headimg, u.pay_points, u.user_rank, u.rank_points, '.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        $_SESSION['headimg'] = \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($row['headimg']);
   ...
   foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            ...
            case 9:
                /* 易物下的商品 */
                $GLOBALS['smarty']->assign('goods_barter_' . $row['id'], assign_barter_goods($row['id'], $row['number']));
                // 排序->楼层
                $GLOBALS['smarty']->assign('goods_barter_' . $row['id'] . '_sort_order', ($row['sort_order'] + 1));
                $ext_info = unserialize($row['ext_info']);
                if(empty($ext_info['barter_color'])){
                    $ext_info['barter_color'] = "#E31939";
                }else if(strpos($ext_info['barter_color'], '#') == false){
                    $ext_info['barter_color'] = '#'.$ext_info['barter_color'];
                }
                // 扩展信息
                $GLOBALS['smarty']->assign('goods_barter_' . $row['id'] . '_ext_info', $ext_info);
                break;
        }
    }
}
 ...
 * @return  array       $pager
 */
function get_pager($url, $param, $record_count, $page = 1, $size = 1)
{
    $size = intval($size);
    if ($size <= 1)
    {
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        {
            $keywords = iconv('GBK', 'UTF8//TRANSLIT//IGNORE', $keywords);
        }
        if (EC_CHARSET == 'gbk' && !in_array($searchengine, $gb_search))
        {
            $keywords = iconv('UTF8', 'GBK//TRANSLIT//IGNORE', $keywords);
        }
 ...
 * @param   string  $link       链接文字 支持数组
 * @param   string  $href       链接跳转地址 支持数组
...
function set_affiliate()
{
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), time() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), time() + 3600 * 24); // 过期时间为 1 天
        }
    }
}
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    {
        if($navlist['middle'][$k]['name'] =="精品商城" && empty($cur_url) ){
            $navlist['middle'][$k]['active'] = 1;
        }
        $condition = empty($ctype) ? (strpos($cur_url, $v['url']) === 0 && strlen($cur_url) == strlen($v['url'])) : (strpos($cur_url, $v['url']) === 0 && strlen($cur_url) == strlen($v['url']));
        if ($condition)
        {
            $navlist['middle'][$k]['active'] = 1;
            $noindex = true;
