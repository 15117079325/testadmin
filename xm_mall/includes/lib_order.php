...
function order_goods($order_id)
{
	...
	//手机专享价格  app  jx
    //edit shengl 2017年2月23日19:27:41 新加积分字段查询
...
//            " WHERE order_id = '$order_id'";// 修改 by www.ecshop68.com 增加 package_attr_id 字段
    $sql = "SELECT rec_id, og.goods_id, og.product_id,og.goods_name, g.goods_thumb, og.goods_sn, og.market_price,og.exclusive, og.goods_number, " .
        "og.goods_price, og.goods_attr, og.is_real, og.parent_id, og.is_gift, " .
        "og.goods_price * og.goods_number AS subtotal, og.extension_code, og.order_id, package_attr_id,  " .
        "og.barter_type,og.shopping_money,og.cash_money,og.consume_money,og.shopping_money * og.goods_number AS subshopping,og.cash_money * og.goods_number AS subcash,og.consume_money * og.goods_number AS subconsume  " .
        "FROM " . $GLOBALS['ecs']->table('order_goods') . "as og right join" . $GLOBALS['ecs']->table('goods') .
        "as g on og.goods_id = g.goods_id" .
        " WHERE order_id = '$order_id'";// 修改 by www.ecshop68.com 增加 package_attr_id 字段
//end shengl
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total  = array('real_goods_count' => 0,
                    ...
                    'pay_fee'          => 0,
                    'tax'              => 0,
        //edit shengl 2017年2月23日20:48:22 新增积分总价
                    'shopping_all'     => 0,
                    'cash_all'     => 0,
                    'consume_all'     => 0,
                     'barter_type' =>0
    ...
    foreach ($goods AS $val)
    {
        ...
        $total['goods_id'] = $val['goods_id'];
        $total['barter_type'] = $val['barter_type'];

        //edit shengl 2017年2月23日20:50:04 加积分总价
        if($val['barter_type'] == '1'){
            $total['shopping_all']  += $val['shopping_credit'] * $val['goods_number'];
            $total['cash_all']  =0;
            $total['consume_all']  =0;
        }elseif($val['barter_type'] == '2'){
            $total['shopping_all']  =0;
            $total['cash_all']  += $val['cash_credit'] * $val['goods_number'];
            $total['consume_all']  += $val['consume_credit'] * $val['goods_number'];
        }else{
            $total['shopping_all']  += $val['shopping_credit'] * $val['goods_number'];
            $total['cash_all']  += $val['cash_credit'] * $val['goods_number'];
            $total['consume_all']  += $val['consume_credit'] * $val['goods_number'];
    ...
    //edit shengl 2017年2月23日20:55:35 提前返回，因为没有后续优惠业务，所以暂时不做后面的处理
    return $total;
    //end
...
function get_order_sn()
{
    ...
    return local_date('Ymd',gmtime()) . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
}
...
/* 代码增加_end  By www.68ecshop.com */
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
$sql_wherer = $_SESSION['user_id']>0 ? "b.user_id='". $_SESSION['user_id'] ."' " : "b.session_id = '" . SESS_ID . "' AND b.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			" c.goods_price,c.cash_money, c.consume_money,c.shopping_money,c.barter_type, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
    ...
    //edit shengl 2017年2月23日20:40:00 添加查询字段
    $sql .= "c.cash_money * c.goods_number AS subcash,c.consume_money * c.goods_number AS subconsume,c.shopping_money * c.goods_number AS subshopping ";
    //end shengl

    $sql .= " FROM " . $GLOBALS['ecs']->table('cart') .
    ...
    $amount = "SELECT IFNULL(ma1.money,0) as cash_credit, IFNULL(ma2.money,0) as consume_credit, IFNULL(ma3.money,0) as shopping_credit ".
...
//    $casha =  $GLOBALS['db']->getRow($amount);
    include_once ROOT_PATH . 'includes/lib_tran.php';
    //查出用户积分情况
    $user_info_points = get_user_points($_SESSION['user_id']);
    $casha['cash_credit'] = $user_info_points['tps']['unlimit'];
    $casha['consume_credit'] = $user_info_points['tps']['shopp'];
    $casha['shopping_credit'] = $user_info_points['tps']['coupon'];
    //var_dump($casha);die();   
    /* 格式化价格及礼包商品 */
    foreach ($arr as $key => $value)
    {
        $arr[$key]['formated_market_price'] = price_format($value['market_price'], false);
		$arr[$key]['cash_credit'] = $value['cash_money'];
        $arr[$key]['consume_credit'] = $value['consume_money'];
        $arr[$key]['shopping_credit'] = $value['shopping_money'];
        $arr[$key]['cash_create'] = $casha['cash_credit'];
        $arr[$key]['consume_create'] = $casha['consume_credit'];
        $arr[$key]['shopping_create'] = $casha['shopping_credit'];
        $arr[$key]['cash_money'] = $value['cash_money']* $value['goods_number'];
        $arr[$key]['consume_money'] = $value['consume_money']* $value['goods_number'];
        $arr[$key]['shopping_money'] = $value['shopping_money']* $value['goods_number'];
        $arr[$key]['barter_type'] = $value['barter_type'];
...
//            " AND g.is_delete = 0";
//edit 2017年2月23日10:49:11  添加消费积分  新美积分 添加推荐人 比例分成

    $sql = "SELECT g.goods_name, mq.cash_credit, mq.consume_credit, mq.shopping_credit, mq.belongto, mq.referee_id, mq.referee_percent, g.goods_sn, g.is_on_sale, g.is_real, ".
        "g.market_price, g.cost_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date, ".
        "g.promote_end_date, g.goods_weight, g.integral, g.extension_code, ".
        "g.goods_number, g.is_alone_sale,g.exclusive, g.is_shipping,".
        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
        ...
        " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
        " LEFT JOIN " . $GLOBALS['ecs']->table('mq_goods_extra') . " AS mq ".
        "ON mq.goods_id = g.goods_id ".
        " WHERE g.goods_id = '$goods_id'" .
        " AND g.is_delete = 0";
    //end
    ...
    $parent = array(
	    ...
	    'add_time'   => time(),   //代码增加   By  www.68ecshop.com
        'rec_type'      => CART_GENERAL_GOODS,

        //add 2017年2月23日11:09:39 购物车金额的添加商城类型添加
//        'shopping_money' =>  $goods['shopping_credit'],
//        'cash_money'    =>  $goods['cash_credit'],
//        'consume_money' =>  $goods['consume_credit'],
        'barter_type'    =>  $goods['belongto'],
        'referee_percent'  => $goods['referee_percent'],
        'referee_id' =>  $goods['referee_id']
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $barter_goods_price     = get_barter_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$num'" .
                       " , goods_price = '$goods_price' , shopping_money = '$barter_goods_price[shopping_credit]', cash_money = '$barter_goods_price[cash_credit]',consume_money = '$barter_goods_price[consume_credit]' ".
                       " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                       " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                       " AND extension_code <> 'package_buy' " .
                       "AND rec_type = 'CART_GENERAL_GOODS'";
        ...
        else //购物车没有此物品，则插入
        {
            ...
            if($goods['belongto'] == 1){
                $parent['shopping_money'] = $barter_goods_price['shopping_credit'];
                $parent['cash_money'] = 0;
                $parent['consume_money'] = 0;
            }elseif($goods['belongto'] == 2){
                $parent['shopping_money'] = 0;
                $parent['cash_money'] = $barter_goods_price['cash_credit'];
                $parent['consume_money'] = $barter_goods_price['consume_credit'];
            }else{
                $parent['shopping_money'] = $barter_goods_price['shopping_credit'];
                $parent['cash_money'] = $barter_goods_price['cash_credit'];
                $parent['consume_money'] = $barter_goods_price['consume_credit'];
...
function user_info($user_id)
{
	...
	$sql = "SELECT u.* FROM " . $GLOBALS['ecs']->table('users') .
...
/**
 * 查看用户积分剩余情况
 * 2017年2月27日 00:00:49 by fzp
 * @param   int     $user_id    用户id
 * @return  array   用户积分信息
 */
function user_info_jifen($user_id)
{
    $sql = "SELECT u.user_name, u.user_id, ma1.money as cash, ma2.money as consume, ma3.money as invest, ma4.money as register, 
            ma5.money as share, ma6.money as shopping, ma7.money as useable " . 
            " FROM " . $GLOBALS['ecs']->table('users') . " AS u " . 
            " LEFT JOIN " . $GLOBALS['ecs']->table('mq_users_extra') . " AS mue ON u.user_id = mue.user_id " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma1 ON mue.account_id = ma1.account_id and ma1.account_type = 'cash' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma2 ON mue.account_id = ma2.account_id and ma2.account_type = 'consume' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma3 ON mue.account_id = ma3.account_id and ma3.account_type = 'invest' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma4 ON mue.account_id = ma4.account_id and ma4.account_type = 'register' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma5 ON mue.account_id = ma5.account_id and ma5.account_type = 'share' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma6 ON mue.account_id = ma6.account_id and ma6.account_type = 'shopping' " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('mq_account') . " AS ma7 ON mue.account_id = ma7.account_id and ma7.account_type = 'useable' " .
 ...
 * @param   int     $refund_type    退款方式 1 到帐户优惠券 2 到退款申请（先到优惠券，再申请提款） 3 不处理
...
function get_cart_goods($other='')
{
    ...
    /* 代码增加_start    By   www.68ecshop.com */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
	$sql = "SELECT c.*, g.cat_id,c.cash_money as cash_credit,c.consume_money as consume_credit, g.brand_id, IF(ga.act_id, ga.supplier_id, g.supplier_id) as supplier_id, IF(c.parent_id, c.parent_id, c.goods_id) AS pid  " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //add 2017年2月23日14:05:34 增加新美积分
        $total['cash_credit'] += $row['cash_money'] * $row['goods_number'];
        $total['consume_credit'] += $row['consume_money'] * $row['goods_number'];
        $row['cash_credit']  = $row['cash_credit'] * $row['goods_number'];
        $row['consume_credit']  = $row['consume_credit'] * $row['goods_number'];
        //end
...
function put_virtual_cart($order_id){
    ...
    for($i=0;$i<$goods['goods_number'];$i++){
        $coded_card_sn   = (rand(1000,9999).$i.time());
        $add_date = time();
