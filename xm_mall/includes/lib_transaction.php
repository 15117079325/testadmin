...
function get_profile($user_id)
{
    ...
    $info['face_card']	= \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($infos['face_card']);
    $info['back_card']	= \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($infos['back_card']);
	$info['hand_photo']	= \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($infos['hand_photo']);
    ...
    $info['refusal_reason'] = $infos['refusal_reason'];
    $info['headimg'] = \maqu\filesystem\FileSystemManager::getAdapter()->getFullPath($infos['headimg']);
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status,barter_type FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    $order = $GLOBALS['db']->GetRow($sql);
    ...
    else
    {
        ...
        //如果是精品商城查询推荐人id
            ...
            //插入推荐人提成表
           if ($referee_id_order_goods[0]['referee_id']) {
             foreach ($referee_id_order_goods as $v){
                $sql_jz = "INSERT INTO " . $GLOBALS['ecs']->table('mq_referee_bonus') . 
                        ...
                        ('" .$v['referee_id'].  "','" .$v['rec_id']. "','" . ($v['cash_money']+$v['consume_money']) * $v['referee_percent'] /100 .
                        "','"  . $v['shopping_money'] * $v['referee_percent'] /100 . "','" . time() . "','"  .$v['referee_percent']. "') ";
                //插入数据库执行
                ...
                $GLOBALS['db']->query($sql_jz);
                 ...
                 if($v['barter_type']==1){
                     ...
                     if($bonus_amount>0){
                         ...
                         $update_smoney =$GLOBALS['db']->query($new_shopping_money_sql);
                         if($update_smoney){
                             ...
                             $time = time();
                             $account_log = 'INSERT INTO ' . $GLOBALS['ecs']->table('mq_account_log') . "(`account_id`, `account_type`, `money`, `change_money`, `change_desc`, `change_time`, `change_type`,`income_type`) VALUES ('$user_account_id', 'shopping', '$new_shopping_money' , '$bonus_amount', '易业商城商品推荐提成：订单号".$order['order_sn']."', '$time', '8','+')";
                             $GLOBALS['db']->query($account_log);
                         }
                     }
                     ...
                     if($bonus_amount>0){
                         ...
                         $update_cmoney = $GLOBALS['db']->query($new_shopping_money_sql);
                         if($update_cmoney){
                             $user_account_sql = "SELECT IFNULL(ma1.account_id,0) as ma1_account_id ".
                                 " FROM " . $GLOBALS['ecs']->table('mq_users_extra') ." AS b ".
                                 " LEFT JOIN " . $GLOBALS['ecs']->table('mq_account')." AS ma1 ON b.account_id = ma1.account_id and ma1.account_type = 'cash' ".
                             ...
                             $time = time();
                             $account_log = 'INSERT INTO ' . $GLOBALS['ecs']->table('mq_account_log') . "(`account_id`, `account_type`, `money`, `change_money`, `change_desc`, `change_time`, `change_type`,`income_type`) VALUES ('$user_account_id', 'cash', '$new_cash_money' , '$bonus_amount', '精品商城商品推荐提成：订单号".$order['order_sn']."', '$time', '8','+')";
                             $GLOBALS['db']->query($account_log);
                         }
                     }
                 }
            ...
            //edit 确认收货
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED .
        "',shipping_time_end = '" . time() . "' WHERE order_id = '$order_id'";
        if ($GLOBALS['db']->query($sql))
        {
