...
function get_user_orders_1($user_id, $num = 10, $start = 0,$where='')
{
    ...
    {	//var_dump($row['order_status'],OS_UNCONFIRMED);die();
        if ($row['order_status'] ==  OS_UNCONFIRMED)//0
        {
			//echo  2222;die();
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)//5
        {
            /* 对配送状态的处理 */
			//var_dump($row['shipping_status']);die();
            if ($row['shipping_status'] ==  SS_SHIPPED)//1
            {
				...
				@$row_time =ceil( $okgoods_time - (time()/86400- $row['shipping_time']/86400));
                @$row['handler'] = "<div class='clearfix'><i class='endtime-icon fl'></i><em class='endtime-text fl' style=font-size:11px;>还剩" . $row_time . "天自动收货</em></div><a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$back_info.$GLOBALS['_LANG']['confirm_received']."')) return false;\" style='display:inline-block;background:#E31939;color:#fff;padding:3px 5px ;margin:3px 0px;'>".$GLOBALS['_LANG']['received']."</a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)//2
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)//0
                {
		...
		$row['weixiu_time'] = ($weixiu_time - (date('d',time()) - date('d',$order['shipping_time_end'])) <= 0) ? 0 : 1;
		...
		$arr[$row['order_id']] = array('order_id'       => $row['order_id'],
			...
			//edit shengl 2017年2月23日17:02:24 添加了新的字段
						'shopping_money_all' => $row['shopping_money_all'],
						'cash_money_all' => $row['cash_money_all'],
						'consume_money_all' => $row['consume_money_all']
			//end shengl
		);
    }
	//var_dump($arr);
