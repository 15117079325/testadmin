...
//	}
	...
	function __construct ($cfg)
	{
		...
		$this->user_table_mq = 'mq_users_extra';
	...
	function check_user ($username, $password = null)
	{
		...
		{
			$post_username = iconv('UTF8', $this->charset.'//TRANSLIT//IGNORE', $username);
		}
		else
		{
		...
		{
			/*edit 增加禁用字段 判断用户是否被禁用 2017年3月1日 15:47:16 by fzp */
			
			// $sql = "SELECT user_id, password, salt,ec_salt " . " FROM " . $this->table($this->user_table) . " WHERE user_name='$post_username'";
			// $sql = "SELECT u.user_id, u.password, u.salt,u.ec_salt " . " FROM " . $this->table($this->user_table) ." AS u LEFT JOIN " .
			// 		$this->table($this->user_table_mq) . " AS mue ON u.user_id = mue.user_id" . " WHERE u.user_name='$post_username' AND mue.user_status=1";
			$sql = "SELECT u.user_id, u.password, u.salt,u.ec_salt,mue.user_status" . " FROM " . $this->table($this->user_table) ." AS u LEFT JOIN " .
				$this->table($this->user_table_mq) . " AS mue ON u.user_id = mue.user_id" . " WHERE u.user_name='$post_username'";
