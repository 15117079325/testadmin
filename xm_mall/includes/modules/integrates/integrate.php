...
class integrate
{
	 ...
	 * @return void
	 */
	// add 增加用户禁用判断 2017年3月1日 17:21:33 by fzp
    function login ($username, $password, $remember = null)
    {
//		var_dump($username);
//		if(is_mobile_phone($username))
//		{
//			$sql = "select " . $this->field_name . " from " . $this->table($this->user_table) . " where " . $this->field_mobile_phone . "='" . $username . "' or " . $this->field_name . "='" . $username . "'";
...
//		}
        ...
        $check_result = $this->check_user($username, $password);
        if($check_result > 0)
        {
            if($this->need_sync)
            {
	...
	function remove_user ($id)
	{
		...
		if($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
		{
			...
			if($col)
			{
				/*add  2017年2月23日 13:22:41 by fzp*/
				$sql2 = "SELECT account_id FROM " . $GLOBALS['ecs']->table('mq_users_extra') . " AS mue " . 'LEFT JOIN' . 
					$GLOBALS['ecs']->table('users') . ' AS u ON mue.user_id = u.user_id ' . "WHERE " . db_create_in($col, 'u.user_id');
				$account_id = $GLOBALS['db']->getCol($sql2);
				//删除用户积分
				$sql3 = "DELETE FROM " . $GLOBALS['ecs']->table('mq_account') . " WHERE " . db_create_in($account_id,'account_id');
				$GLOBALS['db']->query($sql3);
				//删除用户积分关联
				$sql4 = "DELETE FROM " . $GLOBALS['ecs']->table('mq_users_extra') . ' WHERE ' .db_create_in($col,'user_id');
	...
	{
		$sql = "SELECT " . $this->field_id . " AS user_id," . $this->field_name . " AS user_name," . $this->field_email . " AS email," . $this->field_gender . " AS sex," . $this->field_email_validated . " AS email_validated, " . $this->field_mobile_phone . " AS mobile_phone, " .
				$this->field_mobile_validated . " AS mobile_validated, " . $this->field_bday . " AS birthday," . $this->field_reg_date .
				" AS reg_time, " . $this->field_pass . " AS password " . " FROM " . $this->table($this->user_table) .
	 ...
	 * @return int
	 */
	function mq_add_user ($username, $password, $email, $mobile_phone,$invite_code='',$gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
	{
		/* 将用户添加到整合方 */
		if($this->check_user($username) > 0)
		{
...
//		}
		...
		$this->db->query($sql);
		$info = $this->db->getRow("SELECT * FROM " .  $this->table($this->user_table) . " WHERE user_name='$post_username'");

		$account_id = $this->account_id();
		while (true) {
			$count = $this->db->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('mq_users_extra') . " AS ue WHERE ue.account_id = '$account_id' ");
			if ($count > 0) {
				$account_id =  $this->account_id();
			} else {
				break;
		...
		/*edit 去掉验证码表查询users的手机号码作为邀请码*/
		$invite_info = $this->db->getRow("SELECT * FROM " .  $GLOBALS['ecs']->table('users') . " WHERE mobile_phone='$invite_code'");
		$invite_user_id = $invite_info["user_id"];
		...
		//邀请人layer
		$user_layer = $this->db->getOne("SELECT layer FROM " .  $GLOBALS['ecs']->table('mq_users_extra') . " WHERE user_id='$invite_user_id'");

		//同线同级最后一个layer
		$layer = $this->db->getOne("SELECT ifnull(layer,'') FROM " .  $GLOBALS['ecs']->table('mq_users_extra') . " WHERE invite_user_id='$invite_user_id' ORDER BY layer DESC limit 0,1");

		if($layer != '') {
			$new_layer = $layer + '1';
			$new_layer = sprintf('%0'.strlen($layer).'d',$new_layer);
		}
		else{
			$new_layer = $user_layer . '001';
		}

		$sql1 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_users_extra') .
				" (user_id,user_cx_rank,account_id,is_referee,invite_code,pay_password,invite_user_id,layer)"
				. " VALUES ('$info[user_id]', '0', '$account_id', '0','$invite_code','$post_password','$invite_user_id','$new_layer')";

		$this->db->query($sql1);
		$creat_at = time();
		$sql_account1 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'cash','0','0','1','$creat_at')";
		$sql_account2 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'consume','0','0','1','$creat_at')";
		$sql_account3 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'invest','0','0','1','$creat_at')";
		$sql_account4 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'register','0','0','1','$creat_at')";
		$sql_account5 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'share','0','0','1','$creat_at')";
		$sql_account6 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'shopping','0','0','1','$creat_at')";
		$sql_account7 = "INSERT INTO " . $GLOBALS['ecs']->table('mq_account') . " (`account_id`,`account_type`,`money`,`frozen_money`,`status`,`create_at`)" . " VALUES ('$account_id', 'useable','0','0','1','$creat_at')";
		$this->db->query($sql_account1);
		$this->db->query($sql_account2);
		$this->db->query($sql_account3);
		$this->db->query($sql_account4);
		$this->db->query($sql_account5);
		$this->db->query($sql_account6);
		$this->db->query($sql_account7);
		$sql_invite_code = 'UPDATE ' . $this->table('mq_invite_code') . ' SET usage_count = (SELECT temp.usage_count+1 FROM (
SELECT IFNULL(MAX(usage_count),0) AS usage_count
	FROM ' . $GLOBALS['ecs']->table('mq_invite_code') . " WHERE code ='" . $invite_code . "') AS temp ) WHERE code ='" . $invite_code . "'";
		$this->db->query($sql_invite_code);
