 ...
 * @param isnowbuy 是否立即购买
 * @param extCode extCode 扩展代码，可以自定义一些扩展属性，例如：'pre_sale'
 */
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (isSelectAttr(spec_arr)) {
		...
		goods.number = number;	//库存
		goods.parent = (typeof (parentId) == "undefined") ? 0 : parseInt(parentId);	//父节点
		goods.extCode = extCode;	//extension_code
...
function getSelectedAttributes(formBuy) {
	...
	for (i = 0; i < formBuy.elements.length; i++) {
		...
		if (prefix == 'spec_' && (((formBuy.elements[i].type == 'radio' || formBuy.elements[i].type == 'checkbox')
				&& formBuy.elements[i].checked) || formBuy.elements[i].tagName == 'SELECT')) {
...
function addToCartResponse(result) {
	...
	if (result.error > 0) {
		...
		if (result.error == 1){
			...
			//$('.pop-mask').toggle();
			alert('货物已经售完或者最后一件货物已经在您购物车中');

		}
		// 没选规格，弹出属性选择框
		else if (result.error == 6) {
			if(barter_type == 2){
				openBarterSpeDiv(result.message, result.goods_id, result.parent);
			}else{
...
// 生成属性选择层
function openBarterSpeDiv(message, goods_id, parent) {
	var _id = "speDiv";
	var m = "mask";
	if (docEle(_id))
		document.removeChild(docEle(_id));
