...
function checkMobile(sMobile){ 
	if(!(/^1[3|4|5|7|8|9][0-9]\d{4,8}$/.test(sMobile))){
		return false; 
	}
	else
	{
...
/* add 2017-02-14  by wj start*/
/**
 * 验证邀请码,第一步合法性验证， 第二步是否存在验证
 *
 * @param email
 *            验证邮箱：支持邮箱和邮箱对象
 * @param callback
 *            回调函数：true-可以注册 false-不可以注册
 */
function checkInviteCode(invite_code, callback) {
	var submit_disabled = false;

	var invite_codeObj = null;

	if (typeof (invite_code) == 'object') {
		invite_codeObj = $(invite_code);
		invite_code = invite_codeObj.val();
	}

	if (invite_code == '') {
		document.getElementById('invite_code_notice').innerHTML = msg_invite_code_blank;
		document.getElementById('invite_code_notice').style.color = '#E31939';
		submit_disabled = true;

		if (invite_codeObj != null) {
			invite_codeObj.focus();
		}

	}
	else if (!Utils.isMobile(invite_code)) {
		document.getElementById('invite_code_notice').innerHTML = msg_invite_code_getshi_not_valid;
		document.getElementById('invite_code_notice').style.color = '#E31939';
		submit_disabled = true;
	...
	if (invite_codeObj == null) {
		checkInviteCodeExist(invite_code, callback);
	} else {
		checkInviteCodeExist(invite_codeObj, callback);
	}
}
/**
 * 检查邀请码是否存在
 ...
 *            回调函数：true-可以注册 false-不可以注册
 */
function checkInviteCodeExist(invite_code, callback) {
	...
	$.post('register.php?act=check_invite_code_exist', {
		invite_code: invite_code
	}, function(result) {
		if (result == 'true') {
			document.getElementById('invite_code_notice').innerHTML = msg_invite_code_valid;
			document.getElementById('invite_code_notice').style.color = '#093';
			document.forms['formUser'].elements['Submit'].disabled = '';
			...
			document.getElementById('invite_code_notice').innerHTML = msg_invite_code_not_valid;
			document.getElementById('invite_code_notice').style.color = '#E31939';
			document.forms['formUser'].elements['Submit'].disabled = 'disabled';
...
function checkUserName(username, callback) {
	...
	if (username == '') {
		document.getElementById('username_notice').innerHTML = msg_username_blank;
		document.getElementById('username_notice').style.color = '#E31939';
		submit_disabled = true;
		...
		document.getElementById('username_notice').innerHTML = msg_username_format;
		document.getElementById('username_notice').style.color = '#E31939';
		submit_disabled = true;
...
function checkUserNameExist(username, callback) {
	...
	$.post('register.php?act=check_username_exist', {
		...
		if (result == 'true') {
			document.getElementById('username_notice').innerHTML = msg_username_valid;
			...
			document.getElementById('username_notice').innerHTML = msg_username_not_valid;
			document.getElementById('username_notice').style.color = '#E31939';
			document.forms['formUser'].elements['Submit'].disabled = 'disabled';
...
function checkCaptcha(captcha, callback) {
	...
	if (captcha == '') {
		document.getElementById('captcha_notice').innerHTML = msg_captcha_blank;
		document.getElementById('captcha_notice').style.color = '#E31939';
		submit_disabled = true;
	...
	if (captchaObj == null) {
		checkCaptchaExist(captcha, callback);
	} else {
		checkCaptchaExist(captchaObj, callback);
	}
}
/**
 * 检查验证码是否正确 by fzp
 ...
 *            回调函数：true-可以注册 false-不可以注册
 */
function checkCaptchaExist(captcha, callback) {
	...
	$.post('register.php?act=check_captcha_exist', {
		captcha: captcha
	}, function(result) {
		if (result == 'true') {
			document.getElementById('captcha_notice').innerHTML = msg_captcha_valid;
			document.getElementById('captcha_notice').style.color = '#093';
			document.forms['formUser'].elements['Submit'].disabled = '';
			...
			document.getElementById('captcha_notice').innerHTML = msg_captcha_not_valid;
			document.getElementById('captcha_notice').style.color = '#E31939';
			document.forms['formUser'].elements['Submit'].disabled = 'disabled';
			//alert('2');
