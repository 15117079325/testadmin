...
/* 优惠券额支付密码_添加_START_www.68ecshop.com */
var selected_pay_code = null;
/* 优惠券额支付密码_添加_END_www.68ecshop.com */
	...
	if (obj.error) {
		...
		try {
			...
			if(obj.show){
				//如果优惠券完全支付订单金额
				$('#pay_div').hide();
				$('#payment_other_input').attr("checked", true).val(pay_balance_id);//默认选择优惠券支付方式
			}else{
				$('#pay_div').show();
...
//add  支付密码 2017年2月27日16:38:19
function open_surplus_window() {
	$.ajax({
		type:"GET",
		url:"flow.php?act=user_balance_money",
		dataType:"JSON",
		async: false,
		success:function(json){
			var the_form = document.getElementById("theForm");
			var cash_create = json['cash_credit'];
			var shopping_create = json['shopping_credit'];
			var consume_create = json['consume_credit'];
			var cash_all = json['cash_all'];
			var consume_all = json['consume_all'];
			var shopping_all = json['shopping_all'];
			var barter_type = json['barter_type'];
			if((barter_type == 1 && shopping_create >= shopping_all) || (barter_type == 2 && cash_create >= cash_all && consume_create >= consume_all ) || (barter_type == 4 && cash_create >= cash_all && consume_create >= consume_all ) ) {
