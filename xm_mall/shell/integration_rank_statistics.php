...
function find_rank_list($rank)
{
    ...
    for ($i = 1; $i < 8; $i++) {
        switch ($i) {
            case 1:
                $table = $GLOBALS['ecs']->table('tps');
                $filed = " x.unlimit";
                foreach ($ret as $key => $value) {
                     $money = get_lower($value['user_id'], 0, $table, $filed, $rank);
                     $self_sql = "SELECT SUM({$filed}) FROM {$table} x  WHERE user_id={$value['user_id']}";
            ...
            case 7:
                ...
                foreach ($ret as $key => $value) {
                    ...
                    $self_sql = "SELECT SUM({$filed}) FROM {$table} x  WHERE user_id={$value['user_id']}";
                     $self_money = $GLOBALS['db']->getOne($self_sql);
                    $money_arr7[$value['user_id']] = $money+$self_money;
                    $GLOBALS['u_invi_money'] = 0;
                };
                break;
    ...
    $time = time();
    $sql = "INSERT INTO `xm_integration_rank` (`user_id`,`user_cx_rank`,`top_user_id`,`type`,`money`,`create_time`) VALUES ";
    ...
    foreach ($money_arr7 as $k => $v) {
        $sql .= "({$k},{$rank},{$top_user_id[$k]},7,$v,{$time}),";
    }
    $sql = rtrim($sql, ',');
    $db->query($sql);
    return true;
...
function get_lower($str, $money = 0, $table, $filed, $rank)
{
    ...
    $sql1 = "SELECT SUM({$filed}) FROM {$table} x LEFT JOIN xm_mq_users_extra e ON x.user_id=e.user_id WHERE e.invite_user_id in ($str) AND e.user_cx_rank<{$rank}";
    ...
    if (count($list) != 0) {
        ...
        if ($ll) {
            ...
            get_lower($str, $inv_c, $table, $filed, $rank);
        }
